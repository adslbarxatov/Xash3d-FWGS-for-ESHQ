...
void WeaponsResource::SelectSlot (int iSlot, int fAdvance, int iDirection)
	...
	if ((gpActiveSel == NULL) || (gpActiveSel == (WEAPON*)1) || (iSlot != gpActiveSel->iSlot))
		...
		if (p && fastSwitch && p3) 
			...
			// ¬ противном случае вз€ть первое из списка
			else
				{
				ServerCmd (p->szName);
			...
			// ќзначает, что отмена произойдЄт, когда позици€ есть, но нет следующего оружи€, а слоты совпадают
			if (p2 || (p3->iSlotPos != p->iSlotPos) || (iSlot != p3->iSlot))
				switch ((p3->iSlot + p3->iSlotPos) % 3)
					{
					default:
						PlaySound ("items/gunpickup3.wav", 1);
						break;
					case 1:
						PlaySound ("items/gunpickup1.wav", 1);
						break;
					case 2:
...
int CHudAmmo::Draw (float flTime)
	...
	// ESHQ: цвет дл€ оставшегос€ количества патронов
	// ! не удаЄтс€ найти ссылку на MAXCLIP дл€ текущего оружи€
