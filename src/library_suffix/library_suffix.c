...
/***
============
Q_PlatformStringByID [FWGS, 01.07.26]
...
/***
============
Q_ArchitectureStringByID [FWGS, 01.07.26]
...
/***
====================
COM_StripIntelSuffix [FWGS, 01.07.26]
...
void COM_StripIntelSuffix (char *out)
	...
	/*if ((suffix[1] == 'i') && (suffix[2] >= '3') && (suffix[2] <= '6') &&
		(suffix[3] == '8') && (suffix[4] == '6'))*/
	if ((suffix[1] == 'i') &&
...
int COM_GenerateLibraryName (char *out, size_t size,
	...
	if (((platform == PLATFORM_WIN32) || (platform == PLATFORM_LINUX) || (platform == PLATFORM_APPLE)))
		...
		{
		len = snprintf (out, size, "%s%s_%s_%s.%s", prefix, name, Q_PlatformStringByID (platform),
			Q_ArchitectureStringByID (arch, abi, endianness, is64), ext);
		}
