...
#define STEP_CONCRETE	0		// Бетон
#define STEP_METAL		1		// Метал
#define STEP_DIRT		2		// Песок/гравий
#define STEP_VENT		3		// Вентилляция
#define STEP_GRATE		4		// Металлическая решётка
#define STEP_TILE		5		// Плитка
#define STEP_SLOSH		6		// Лужа
#define STEP_WADE		7		// Вода
#define STEP_LADDER		8		// Лестница
#define STEP_WOOD		9		// Дерево
#define STEP_SNOW		10		// Снег
#define STEP_GLASS		11		// Стекло
...
void PM_WalkMove ()
	...
	// ESHQ: исправлен завышенный коэффициент, не пропускавший случаи с низким трением (например, лёд – 5%)
	// и не дававший полностью остановившемуся игроку сдвинуться с места без прыжка
	if (spd < 0.1f)
		{
...
void PM_WaterJump (void)
	{
	if (pmove->waterjumptime > 500)		// Значение 10000 было неадекватно большим
		pmove->waterjumptime = 500;
...
void PM_Jump (void)
	...
	if (pmove->waterlevel >= 2)
		...
		if (pmove->flSwimTime <= 0)
			...
			switch (pmove->RandomLong (0, 3))
				{
				case 0:
					pmove->PM_PlaySound (CHAN_BODY, "player/pl_wade1.wav", 1, ATTN_MEDIUM, 0, PITCH_NORM);
					break;
				case 1:
					pmove->PM_PlaySound (CHAN_BODY, "player/pl_wade2.wav", 1, ATTN_MEDIUM, 0, PITCH_NORM);
					break;
				case 2:
					pmove->PM_PlaySound (CHAN_BODY, "player/pl_wade3.wav", 1, ATTN_MEDIUM, 0, PITCH_NORM);
					break;
				case 3:
					pmove->PM_PlaySound (CHAN_BODY, "player/pl_wade4.wav", 1, ATTN_MEDIUM, 0, PITCH_NORM);
					break;
				}
			}
	...
	if (tfc)
		{
		pmove->PM_PlaySound (CHAN_BODY, "player/plyrjmp8.wav", 0.5, ATTN_MEDIUM, 0, PITCH_NORM);
		}
	else
		{
