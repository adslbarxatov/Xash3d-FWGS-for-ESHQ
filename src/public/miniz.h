...
coroutine: see tinfl_decompress(). It supports decompression into a 32KB (or larger power of 2) wrapping buffer,
or into a memory
...
// Set MINIZ_USE_UNALIGNED_LOADS_AND_STORES to 1 on CPU's that permit efficient integer loads
// and stores from unaligned addresses
#define MINIZ_USE_UNALIGNED_LOADS_AND_STORES 1
...
// Set MINIZ_HAS_64BIT_REGISTERS to 1 if operations on 64-bit integers are reasonably
// fast (and don't involve compiler generated calls to helper functions)
#define MINIZ_HAS_64BIT_REGISTERS 1
#else
...
// mz_free() internally uses the MZ_FREE() macro (which by default calls free() unless you've modified
// the MZ_MALLOC macro) to release a block allocated from the heap
...
//  and MINIZ_LITTLE_ENDIAN are defined.)
// Return values:
//  MZ_OK on success
...
//  or -MZ_DEFAULT_WINDOW_BITS (raw deflate/no header or footer)
//  mem_level must be between [1, 9] (it's checked but ignored by miniz.c)
MINIZ_EXPORT int mz_deflateInit2 (mz_streamp pStream, int level, int method, int window_bits, int mem_level, int strategy);

// Quickly resets a compressor without having to reallocate anything. Same as calling mz_deflateEnd() followed
// by mz_deflateInit()/mz_deflateInit2()
MINIZ_EXPORT int mz_deflateReset (mz_streamp pStream);

// mz_deflate() compresses the input to output, consuming as much of the input and producing as much output as possible.
// Parameters:
//  pStream is the stream to read from and write to. You must initialize/update the next_in, avail_in,
//  next_out, and avail_out members.
//  flush may be MZ_NO_FLUSH, MZ_PARTIAL_FLUSH/MZ_SYNC_FLUSH, MZ_FULL_FLUSH, or MZ_FINISH.
// Return values:
//  MZ_OK on success (when flushing, or if more input is needed but not available, and/or there's more output
...
//  MZ_BUF_ERROR if no forward progress is possible because the input and/or output buffers are empty.
//  (Fill up the input buffer or free up some output space and try again.)
...
// mz_deflateBound() returns a (very) conservative upper bound on the amount of data that could
// be generated by deflate(), assuming flush is set to only MZ_NO_FLUSH or MZ_FINISH
...
// Quickly resets a compressor without having to reallocate anything. Same as calling mz_inflateEnd()
// followed by mz_inflateInit()/mz_inflateInit2()
MINIZ_EXPORT int mz_inflateReset (mz_streamp pStream);

// Decompresses the input stream to the output, consuming only as much of the input as needed,
// and writing as much to the output as possible.
// Parameters:
//  pStream is the stream to read from and write to. You must initialize/update the next_in, avail_in,
//  next_out, and avail_out members.
//  flush may be MZ_NO_FLUSH, MZ_SYNC_FLUSH, or MZ_FINISH.
//  On the first call, if flush is MZ_FINISH it's assumed the input and output buffers are both sized
//  large enough to decompress the entire stream in a single call (this is slightly faster).
//  MZ_FINISH implies that there are no more source bytes available beside what's already in the input buffer,
//  and that the output buffer is large enough to hold the rest of the decompressed data.
// Return values:
//  MZ_OK on success. Either more input is needed but not available, and/or there's more output to be
//  written but the output buffer is full.
//  MZ_STREAM_END if all needed input has been consumed and all output bytes have been written. For zlib
//  streams, the adler-32 of the decompressed data has also been verified.
//  MZ_STREAM_ERROR if the stream is bogus.
//  MZ_DATA_ERROR if the deflate stream is invalid.
//  MZ_PARAM_ERROR if one of the parameters is invalid.
//  MZ_BUF_ERROR if no forward progress is possible because the input buffer is empty but the inflater needs
//  more input to continue, or if the output buffer is not large enough. Call mz_inflate() again
...
// Redefine zlib-compatible names to miniz equivalents, so miniz.c can be used as a drop-in replacement
// for the subset of zlib that miniz.c supports.
...
// Set TDEFL_LESS_MEMORY to 1 to use less memory (compression will be slightly slower,
// and raw/dynamic blocks will be output more frequently)
#define TDEFL_LESS_MEMORY 0

// tdefl_init() compression flags logically OR'd together (low 12 bits contain the max.
// number of probes per dictionary search):
// TDEFL_DEFAULT_MAX_PROBES: The compressor defaults to 128 dictionary probes per dictionary
...
// TDEFL_NONDETERMINISTIC_PARSING_FLAG: Enable to decrease the compressor's initialization time to the
// minimum, but the output may vary from run to run given the same input (depending on the contents of memory).
...
//  flags: The max match finder probes (default is 128) logically OR'd against the above flags. Higher
//  probes are slower but improve compression.
// On return:
...
//  The image pitch in bytes per scanline will be w*num_chans. The leftmost pixel on the top scanline
...
//  The caller must mz_free() the returned heap block (which will typically be larger than *pLen_out)
...
// Output stream interface. The compressor uses this interface to write compressed data.
// It'll typically be called TDEFL_OUT_BUF_SIZE at a time
typedef mz_bool (*tdefl_put_buf_func_ptr)(const void *pBuf, int len, void *pUser);

// tdefl_compress_mem_to_output() compresses a block to an output stream. The above helpers
...
// TDEFL_OUT_BUF_SIZE MUST be large enough to hold a single entire compressed output block
// (using static/fixed Huffman codes)
...
// flexible enough. The low-level functions don't make any heap allocations, unlike the above helper functions
typedef enum
	{
...
// pBut_buf_func: If NULL, output data will be supplied to the specified callback. In this case, the user
// should call the tdefl_compress_buffer() API for compression.
// If pBut_buf_func is NULL the user should always call the tdefl_compress() API.
// flags: See the above enums (TDEFL_HUFFMAN_ONLY, TDEFL_WRITE_ZLIB_HEADER, etc.)
MINIZ_EXPORT tdefl_status tdefl_init (tdefl_compressor *d, tdefl_put_buf_func_ptr pPut_buf_func, void *pPut_buf_user, int flags);

// Compresses a block of data, consuming as much of the specified input buffer as possible, and writing
// as much compressed data to the specified output buffer as possible
...
typedef enum
	{
	// This flags indicates the inflator needs 1 or more input bytes to make forward progress,
	// but the caller is indicating that no more are available. The compressed data
	// is probably corrupted. If you call the inflator again with more bytes it'll try to continue
	// processing the input but this is a BAD sign (either the data is corrupted or you called it incorrectly).
	...
	// call it again without resetting via tinfl_init() it it'll just keep on returning the same status
	// failure code
	...
	// of uncompressed data to the caller. I've been assuming you know how much uncompressed data to expect
...
typedef struct
	...
	// File's uncompressed size. Note, I've seen some old archives where directory entries
	// had 512 bytes for their uncompressed sizes, but when you try to unpack them you actually get 0 bytes
...
typedef enum
	...
	// if enabled, mz_zip_reader_locate_file() will be called on each file as its validated to
	// ensure the func finds the file in the central dir (intended for testing)
	...
	// always use the zip64 file format, instead of the original zip file format with automatic
...
// The archive is assumed to be archive_size bytes long. If archive_size is 0, then the entire rest
// of the file is assumed to contain the archive.
...
// MZ_TRUE if the compression method is supported, and the file is not encrypted, and the file
// is not a compressed patch file
...
// and the optional descriptor following the compressed data vs. the data in the central directory.
// It also validates that each file can be successfully uncompressed unless the MZ_ZIP_FLAG_VALIDATE_HEADERS_ONLY
// is specified
...
// so it can be reopened for writing. If the file can't be reopened, mz_zip_reader_end() will be called.
// For archives opened using mz_zip_reader_init_mem, the memory block must be growable using
// the realloc callback (which defaults to realloc unless you've overridden it).
...
// level_and_flags - compression level (0-10, see MZ_BEST_SPEED, MZ_BEST_COMPRESSION, etc.) logically OR'd with zero
...
// Like mz_zip_writer_add_mem(), except you can specify a file comment field, and optionally supply the function
...
#ifndef MINIZ_NO_STDIO
// Adds the contents of a disk file to an archive. This function also records the disk file's modified
// time into the archive.
// level_and_flags - compression level (0-10, see MZ_BEST_SPEED, MZ_BEST_COMPRESSION, etc.) logically
// OR'd with zero or more mz_zip_flags, or just set to MZ_DEFAULT_COMPRESSION
...
// Note this is NOT a fully safe operation. If it crashes or dies in some way your archive can be left in
// a screwed up state (without a central directory).
// level_and_flags - compression level (0-10, see MZ_BEST_SPEED, MZ_BEST_COMPRESSION, etc.) logically OR'd with
// zero or more mz_zip_flags, or just set to MZ_DEFAULT_COMPRESSION.
// TODO: Perhaps add an option to leave the existing central dir in place in case the add dies? We could then
// truncate the file (so the old central dir would be at the end) if something goes wrong
