...
/***
=============
SV_SetMinMaxSize [FWGS, 01.07.26]
...
void SV_CopyTraceToGlobal (trace_t *trace)
	...
	if (SV_IsValidEdict (trace->ent))
		...
		svgame.globals->trace_ent = svgame.edicts;
	}
...
/***
=============
SV_CheckClientVisiblity [FWGS, 01.07.26]
...
static int SV_Multicast (int dest, const vec3_t origin, const edict_t *ent, qboolean usermessage, qboolean filter)
	...
	for (j = 0, cl = current; j < numclients; j++, cl++)
		...
		if (specproxy) MSG_WriteBits (&sv.spec_datagram, MSG_GetData (&sv.multicast),
			MSG_GetNumBitsWritten (&sv.multicast));
		else if
			(reliable) MSG_WriteBits (&cl->netchan.message, MSG_GetData (&sv.multicast),
				MSG_GetNumBitsWritten (&sv.multicast));
		else
			MSG_WriteBits (&cl->datagram, MSG_GetData (&sv.multicast),
				MSG_GetNumBitsWritten (&sv.multicast));
		numsends++;
		}
...
/***
=======================
SV_CreateStaticEntity [FWGS, 01.07.26]
...
/***
=================
SV_RestartStaticEnts [FWGS, 01.07.26]
...
/***
=================
SV_RestartAmbientSounds [FWGS, 01.07.26]
...
void SV_RestartAmbientSounds (void)
	...
	for (int i = 0; i < nSounds; i++)
		...
		SV_StartSound (SV_PEntityOfEntIndex (si->entnum, true), CHAN_STATIC, si->name, si->volume, 
...
/***
=================
SV_RestartDecals [FWGS, 01.07.26]
...
void SV_RestartDecals (void)
	...
	for (int i = 0; i < numdecals; i++)
		...
		if (!FBitSet (entry->flags, FDECAL_STUDIO))
			SV_CreateDecal (msg, entry->position, decalIndex, entry->entityIndex, modelIndex,
...
/***
==============
SV_CreateNamedEntity [FWGS, 01.05.24]
...
/***
=========
SV_FindEntityByString [FWGS, 01.07.26]
=========
***/
static edict_t *SV_FindEntityByString (edict_t *pStartEdict, const char *pszField, const char *pszValue)
	...
	for (int i = 0; i < HLARRAYSIZE (gEntvarsDescription); i++)
		{
...
/***
=================
pfnFindEntityInSphere [FWGS, 01.07.26]
...
/***
=================
SV_CheckClientPVS [FWGS, 01.07.26]
...
static int SV_CheckClientPVS (int check, qboolean bMergePVS)
	...
	if (pvs)
		memcpy (clientpvs, pvs, world.visbytes);
...
static edict_t *GAME_EXPORT pfnFindClientInPVS (edict_t *pEdict)
	...
	if (false && mod && (mod->type == mod_brush) && !FBitSet (mod->flags, MODEL_HAS_ORIGIN))
		{
...
/***
=================
pfnEntitiesInPVS [FWGS, 01.07.26]
=================
***/
...
int SV_BuildSoundMsg (sizebuf_t *msg, edict_t *ent, int chan, const char *sample, int vol, float attn, 
	...
	if ((sample[0] == '!') && Q_isdigit (sample + 1))
		...
		// [FWGS, 01.07.24] precache_sound can be used twice: cache sounds when loading
...
=================
***/
static void GAME_EXPORT pfnEmitAmbientSound (edict_t *ent, float *pos, const char *sample, float vol, float attn,
	int flags, int pitch)
	{
	int	msg_dest;
...
=================
***/
static void GAME_EXPORT pfnTraceLine (const float *v1, const float *v2, int fNoMonsters, edict_t *pentToSkip,
	TraceResult *ptr)
	{
	trace_t	trace;
...
=================
***/
static void GAME_EXPORT pfnTraceHull (const float *v1, const float *v2, int fNoMonsters, int hullNumber,
	...
	trace = SV_Move (v1, sv.worldmodel->hulls[hullNumber].clip_mins, sv.worldmodel->hulls[hullNumber].clip_maxs,
		v2, fNoMonsters, pentToSkip, false);
	SV_ConvertTrace (ptr, &trace);
	}
...
=============
***/
static int GAME_EXPORT pfnTraceMonsterHull (edict_t *pEdict, const float *v1, const float *v2, int fNoMonsters,
	edict_t *pentToSkip, TraceResult *ptr)
	{
	qboolean	monsterClip;
	trace_t		trace;
...
=============
***/
static void GAME_EXPORT pfnTraceSphere (const float *v1, const float *v2, int fNoMonsters, float radius,
	edict_t *pentToSkip, TraceResult *ptr)
	{
	}
...
static void GAME_EXPORT pfnLightStyle (int style, const char *val)
	...
	// set correct style
	SV_SetLightStyle (style, val, 0.0f);
	}

/***
=================
pfnDecalIndex [FWGS, 01.07.26]
...
static void GAME_EXPORT pfnMessageBegin (int msg_dest, int msg_num, const float *pOrigin, edict_t *ed)
	...
	if (iSize == -1)
		...
		{
		svgame.msg_size_index = -1;	// message has constant size
		}
	...
	svgame.msg_trace = (sv_trace_messages.value != 0) && (msg_num > svc_lastmsg) &&
...
static void GAME_EXPORT pfnMessageEnd (void)
	...
	if (svgame.msg_index < 0)
		{
		if (((svgame.msg_index == -svc_finale) || (svgame.msg_index == -svc_cutscene)) &&
			(svgame.msg_realsize == 0))
			MSG_WriteChar (&sv.multicast, 0);	// write null string
		}

	if (!VectorIsNull (svgame.msg_org))
		org = svgame.msg_org;
...
static void GAME_EXPORT pfnWriteByte (int iValue)
	{
	if (iValue == -1)
		iValue = 0xFF;	// convert char to byte
...
=============
***/
static void GAME_EXPORT pfnBuildSoundMsg (edict_t *pSource, int chan, const char *samp, float fvol, float attn,
	int fFlags, int pitch, int msg_dest, int msg_type, const float *pOrigin, edict_t *pSend)
	{
...
static void SV_AllocStringPool (void)
	...
	while (start - base > INT_MIN)
		{
		void	*mapptr = mmap ((void *)((unsigned long)start & ~(pagesize - 1)), arrlen, PROT_READ | PROT_WRITE,
			flags, 0, 0);
...
/***
=============
pfnPEntityOfEntIndex [FWGS, 01.05.24]
=============
***/
...
/***
=============
pfnFindEntityByVars [FWGS, 01.07.26]
...
=============
***/
static void GAME_EXPORT pfnFadeClientVolume (const edict_t *pEdict, int fadePercent, int fadeOutSeconds,
	int holdTime, int fadeInSeconds)
	{
	sv_client_t	*cl;
...
/***
=============
pfnRunPlayerMove [FWGS, 01.07.26]
=============
***/
static void GAME_EXPORT pfnRunPlayerMove (edict_t *pClient, const float *viewangles, float fmove, float smove,
...
/***
=============
pfnNumberOfEntities [FWGS, 01.07.26]
...
/***
=============
pfnGetInfoKeyBuffer [FWGS, 01.05.24]
=============
***/
...
static void GAME_EXPORT pfnSetValueForKey (char *infobuffer, char *key, char *value)
	...
	if (infobuffer == svs.localinfo)
		...
		Con_Printf (S_ERROR "can't set client keys with SetValueForKey\n");
	}
...
static const char *GAME_EXPORT pfnGetPhysicsKeyValue (const edict_t *pClient, const char *key)
	...
	// [FWGS, 01.07.24] pfnUserInfoChanged passed
...
void GAME_EXPORT SV_PlaybackEventFull (int flags, const edict_t *pInvoker, word eventindex, float delay, float *origin,
	...
	for (int slot = 0; slot < svs.maxclients; slot++, cl++)
		...
		// [FWGS, 01.07.24] a1ba: GoldSrc never cleans up host_client pointer (similar to sv.current_client)
...
/***
=============
pfnCheckVisibility [FWGS, 01.07.26]
=============
***/
...
/***
=============
pfnForceUnmodified [FWGS, 01.07.26]
=============
***/
	...
	if (sv.state == ss_loading)
		...
		for (int i = 0; i < MAX_MODELS; i++)
			...
			if (!pc->filename)
				{
				if (mins)
					VectorCopy (mins, pc->mins);
				if (maxs)
					VectorCopy (maxs, pc->maxs);
...
static qboolean GAME_EXPORT pfnVoice_SetClientListening (int iReceiver, int iSender, qboolean bListen)
	...
	if (bListen) 
		SetBits (svs.clients[iSender].listeners, BIT (iReceiver));
	else 
		ClearBits (svs.clients[iSender].listeners, BIT (iReceiver));
...
static enginefuncs_t gEngfuncs =
	...
	pfnEntOffsetOfPEntity,
	pfnIndexOfEdict,
	pfnPEntityOfEntIndexAllEntities,	// [FWGS, 01.05.24]
	pfnFindEntityByVars,
	pfnGetModelPtr,
	pfnRegUserMsg,
	pfnAnimationAutomove,
	...
	pfnBuildSoundMsg,
	pfnIsDedicatedServer,
	SV_CvarGetPointer,		// [FWGS, 01.07.26]
	pfnGetPlayerWONId,
	(void *)Info_RemoveKey,
	pfnGetPhysicsKeyValue,
	pfnSetPhysicsKeyValue,
	...
	while (1)
		...
		if (numpairs > HLARRAYSIZE (pkvd))
			...
			if (classname)
				...
				Con_Printf (S_ERROR "%s: too many keyvalue pairs!\n", __func__);
			break;
			}
		}
...
qboolean SV_LoadProgs (const char *name)
	...
	// make sure that physic functions is cleared
	memset (&svgame.physFuncs, 0, sizeof (svgame.physFuncs));

	// revert fix for pfnPEntityOfEntIndex to be compatible with GoldSrc games that rely on this bug
