...
/***
================
SV_CheckIPRestrictions [FWGS, 01.03.25]
...
/***
================
SV_FindEmptySlot [FWGS, 01.07.26]
...
static void SV_ConnectClient (netadr_t from)
	...
	for (int i = 0; i < svs.maxclients; i++)
		...
		if (NET_CompareBaseAdr (from, cl->netchan.remote_address) && ((cl->netchan.qport == qport) ||
			(from.port == cl->netchan.remote_address.port)))
			{
			newcl = cl;
...
void SV_KickPlayer (sv_client_t *cl, const char *fmt, ...)
	...
	if (buf[0])
		...
		// [FWGS, 01.12.24]
		if (cl->useragent[0])
			Netchan_OutOfBandPrint (NS_SERVER, cl->netchan.remote_address,
				S2C_ERRORMSG "\nKicked with message:\n%s\n", buf);
		}
	else
		{
...
/***
===============
SV_GetClientIDString [FWGS, 01.03.25]
...
/***
================
SV_TestBandWidth [FWGS, 01.07.26]
================
***/
...
/***
===============
SV_RemoteCommand [FWGS, 01.07.26]
...
/***
===================
SV_EstablishTimeBase [FWGS, 01.07.26]
...
/***
===================
SV_CalcClientTime [FWGS, 01.07.26]
...
/***
===================
SV_FullClientUpdate [FWGS, 01.07.26]
...
// [FWGS, 05.04.26] removed SV_RefreshUserinfo
...
/***
===========
PutClientInServer [FWGS, 01.07.26]
...
static qboolean SV_ShouldUpdateUserinfo (sv_client_t *cl)
	...
	if (host.realtime < cl->userinfo_next_changetime + cl->userinfo_penalty *
		sv_userinfo_penalty_multiplier.value)
		{
		// [FWGS, 01.03.25] player changes userinfo too quick! ignore!
	...
	// [FWGS, 01.03.25] they spammed too fast, increase penalty
...
/***
=================
SV_UserinfoChanged [FWGS, 05.04.26]
...
// [FWGS, 01.04.25] removed SV_UpdateUserinfo_f
...
static qboolean SV_Spawn_f (sv_client_t *cl)
	...
	// [FWGS, 01.05.26] if we are paused, tell the clients
	if (sv.paused)
		{
		MSG_BeginServerCmd (&sv.reliable_datagram, svc_setpause);
		MSG_WriteOneBit (&sv.reliable_datagram, sv.paused);
...
/***
==================
SV_SendBuildInfo_f [FWGS, 01.06.25]
==================
***/
...
/***
==================
SV_ClientStatus_f [FWGS, 01.07.26]
==================
***/
static qboolean SV_ClientStatus_f (sv_client_t *cl)
	...
	SV_ClientPrintf (cl, "map:\t%s at %d x, %d y, %d z\n" "players: %i active (%i max)\n"
		...
		sv.name, (int)origin[0], (int)origin[1], (int)origin[2], clients, svs.maxclients);
	...
	for (int i = 0; i < svs.maxclients; i++)
		...
		SV_ClientPrintf (cl, "%2i %5i %4s %4s %g %s\n",
...
static qboolean SV_EntList_f (sv_client_t *cl)
	...
	for (int i = 0; i < svgame.numEntities; i++)
		...
		if (Cmd_Argc () > 1)
			{
			if (!Q_stricmpext (Cmd_Argv (1), SV_GetString (ent->v.classname)) &&
				!Q_stricmpext (Cmd_Argv (1), SV_GetString (ent->v.targetname)))
				continue;
			}
...
static qboolean SV_EntFire_f (sv_client_t *cl)
	...
	for (; (i < svgame.numEntities) && (count < sv_enttools_maxfire.value); i++)
		...
		// [FWGS, 05.04.26] if user specified not a number, try find such entity
		if (!single)
			{
			if (!Q_stricmpext (Cmd_Argv (1), SV_GetString (ent->v.targetname)) &&
				!Q_stricmpext (Cmd_Argv (1), SV_GetString (ent->v.classname)))
				continue;
			}
...
static qboolean SV_EntCreate_f (sv_client_t *cl)
	...
	// [FWGS, 01.07.26] apply keyvalues if supported
	...
	if (!ent->v.targetname)
		...
		// [FWGS, 05.04.26] i know, it may break strict aliasing rules
	...
	// [FWGS, 01.07.26] apply postspawn keyvales if supported
	if (svgame.dllFuncs.pfnKeyValue)
		{
...
/***
==================
SV_ExecuteClientCommand [FWGS, 01.06.25]
==================
***/
...
static void SV_ParseClientMove (sv_client_t *cl, sizebuf_t *msg)
	...
	// [FWGS, 01.03.25] was player kicked? stop here
	if (cl->state <= cs_zombie)
		return;
...
/***
===================
SV_ParseResourceList [FWGS, 01.07.26]
...
/***
===================
SV_ParseVoiceData [FWGS, 01.07.26]
===================
***/
