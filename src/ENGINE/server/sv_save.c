...
/***
=============
DirectoryExtract [FWGS, 01.07.26]
...
static void ParseSaveTables (SAVERESTOREDATA *pSaveData, SAVE_HEADER *pHeader, int updateGlobals)
	...
	// process SAVE_HEADER
	svgame.dllFuncs.pfnSaveReadFields (pSaveData, "Save Header", pHeader, gSaveHeader, HLARRAYSIZE (gSaveHeader));
	...
	for (i = 0; i < pHeader->lightStyleCount; i++)
		{
		svgame.dllFuncs.pfnSaveReadFields (pSaveData, "LIGHTSTYLE", &light, gLightStyle, HLARRAYSIZE (gLightStyle));
...
static void RestoreDecal (SAVERESTOREDATA *pSaveData, decallist_t *entry, qboolean adjacent)
	...
	// [FWGS, 01.07.26] this can happens if brush entity from previous level was turned into world geometry
...
static void LoadClientState (SAVERESTOREDATA *pSaveData, const char *level, qboolean changelevel, qboolean adjacent)
	...
	svgame.dllFuncs.pfnSaveReadFields (pSaveData, "ClientHeader", &header, gSaveClient, HLARRAYSIZE (gSaveClient));

	// restore decals
	for (i = 0; i < header.decalCount; i++)
		{
		svgame.dllFuncs.pfnSaveReadFields (pSaveData, "DECALLIST", &decalEntry, gDecalEntry, HLARRAYSIZE (gDecalEntry));
	...
	for (i = 0; i < header.soundCount; i++)
		{
		svgame.dllFuncs.pfnSaveReadFields (pSaveData, "SOUNDLIST", &soundEntry, gSoundEntry, HLARRAYSIZE (gSoundEntry));
...
static SAVERESTOREDATA *SaveGameState (int changelevel)
	...
	for (i = 0; i < MAX_LIGHTSTYLES; i++)
		...
		svgame.dllFuncs.pfnSaveWriteFields (pSaveData, "LIGHTSTYLE", &light, gLightStyle,
			HLARRAYSIZE (gLightStyle));
		}
...
static qboolean SaveGameSlot (const char *pSaveName, const char *pSaveComment)
	...
	svgame.dllFuncs.pfnSaveWriteFields (pSaveData, "GameHeader", &gameHeader, gGameHeader, HLARRAYSIZE (gGameHeader));
...
static int SaveReadHeader (file_t *pFile, GAME_HEADER *pHeader)
	...
	svgame.dllFuncs.pfnSaveReadFields (pSaveData, "GameHeader", pHeader, gGameHeader, HLARRAYSIZE (gGameHeader));
...
/***
=============
CreateEntityTransitionList [FWGS, 01.07.26]
	...
	for (int i = 0; i < pSaveData->tableCount; i++)
		...
		if (SV_IsValidEdict (pent) && FBitSet (pTable->flags, levelMask))	// screen out the player if he's not to be spawned
			...
			if (FBitSet (pTable->flags, FENTTABLE_GLOBAL))
				...
				svgame.dllFuncs.pfnSaveReadFields (pSaveData, "ENTVARS", &tmpVars, gTempEntvars,
					HLARRAYSIZE (gTempEntvars));
...
/***
==================
SV_GetLatestSave [FWGS, 01.07.26]
