...
qboolean LibraryLoadSymbols (dll_user_t *hInst)
	...
	if (dos_header.e_magic != IMAGE_DOS_SIGNATURE)
		{
		Q_snprintf (errorstring, sizeof (errorstring), "%s does not have a valid dll signature",
			hInst->shortPath);
		goto table_error;
		}
	...
	if (FS_Seek (f, dos_header.e_lfanew, SEEK_SET) == -1)
		{
		Q_snprintf (errorstring, sizeof (errorstring), "%s error seeking for new exe header",
			hInst->shortPath);
		goto table_error;
		}
	...
	if (nt_signature != IMAGE_NT_SIGNATURE)
		{
		Q_snprintf (errorstring, sizeof (errorstring), "%s does not have a valid NT signature",
			hInst->shortPath);
		goto table_error;
		}
	...
	if (FS_Read (f, &optional_header, sizeof (optional_header)) != sizeof (optional_header))
		{
		Q_snprintf (errorstring, sizeof (errorstring), "%s optional header probably corrupted",
			hInst->shortPath);
		goto table_error;
		}
	...
	for (i = 0; i < pe_header.NumberOfSections; i++)
		...
		if (FS_Read (f, &section_header, sizeof (section_header)) != sizeof (section_header))
			{
			Q_snprintf (errorstring, sizeof (errorstring), "%s error during reading section header",
				hInst->shortPath);
			goto table_error;
			}
	...
	if (FS_Seek (f, exports_offset, SEEK_SET) == -1)
		{
		Q_snprintf (errorstring, sizeof (errorstring), "%s does not have a valid exports section",
			hInst->shortPath);
		goto table_error;
		}
	...
	if (FS_Read (f, &export_directory, sizeof (export_directory)) != sizeof (export_directory))
		{
		Q_snprintf (errorstring, sizeof (errorstring), "%s does not have a valid optional header",
			hInst->shortPath);
		goto table_error;
		}
	...
	if (hInst->num_ordinals > MAX_LIBRARY_EXPORTS)
		{
		Q_snprintf (errorstring, sizeof (errorstring), "%s too many exports %i", hInst->shortPath,
			hInst->num_ordinals);
		hInst->num_ordinals = 0;
		goto table_error;
		}
	...
	if (FS_Seek (f, ordinal_offset, SEEK_SET) == -1)
		{
		Q_snprintf (errorstring, sizeof (errorstring), "%s does not have a valid ordinals section",
			hInst->shortPath);
		goto table_error;
		}
	...
	if (FS_Read (f, hInst->ordinals, hInst->num_ordinals * sizeof (word)) != (hInst->num_ordinals * sizeof (word)))
		{
		Q_snprintf (errorstring, sizeof (errorstring), "%s error during reading ordinals table",
			hInst->shortPath);
		goto table_error;
		}
	...
	if (FS_Seek (f, function_offset, SEEK_SET) == -1)
		{
		Q_snprintf (errorstring, sizeof (errorstring), "%s does not have a valid export address section",
			hInst->shortPath);
		goto table_error;
		}
	...
	if (FS_Read (f, hInst->funcs, hInst->num_ordinals * sizeof (dword)) != (hInst->num_ordinals * sizeof (dword)))
		{
		Q_snprintf (errorstring, sizeof (errorstring), "%s error during reading export address section",
			hInst->shortPath);
		goto table_error;
		}
	...
	if (FS_Seek (f, name_offset, SEEK_SET) == -1)
		{
		Q_snprintf (errorstring, sizeof (errorstring), "%s file does not have a valid names section",
			hInst->shortPath);
		goto table_error;
		}
	...
	if (i != hInst->num_ordinals)
		{
		Q_snprintf (errorstring, sizeof (errorstring), "%s error during loading names section",
			hInst->shortPath);
		goto table_error;
		}
