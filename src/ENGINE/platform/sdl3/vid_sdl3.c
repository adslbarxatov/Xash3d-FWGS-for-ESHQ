...
static void VID_SetWindowIcon (SDL_Window *hWnd)
	...
	SDL_Surface	*surface = SDL_CreateSurfaceFrom (icon->width, icon->height, SDL_PIXELFORMAT_RGBA8888,
		icon->buffer, 4 * icon->width);
	FS_FreeImage (icon);
...
static qboolean VID_CreateWindow (const int input_width, const int input_height, window_mode_t window_mode)
	...
	if (window_mode != WINDOW_MODE_WINDOWED)
		Cvar_DirectSetValue (&vid_fullscreen, SDL_GetWindowFullscreenMode (host.hWnd) != NULL ?
			WINDOW_MODE_FULLSCREEN : WINDOW_MODE_BORDERLESS);
...
void VID_Info_f (void)
	...
	Con_Printf ("Window mode: %s" S_DEFAULT "\n",
		FBitSet (flags, SDL_WINDOW_FULLSCREEN) ? S_YELLOW "fullscreen" : S_CYAN "windowed");
...
qboolean VID_SetMode (void)
	...
	switch (err)
		...
		case rserr_invalid_mode:
			Sys_Warn ("%s: invalid mode, engine will run in %dx%d", __func__, vid_state.prev_width,
				vid_state.prev_height);
			break;

		default:
			Sys_Warn ("%s: unknown error, engine will revert to %dx%d", __func__, vid_state.prev_width,
				vid_state.prev_height);
			break;
		}
	...
	switch (window_mode)
		...
		case WINDOW_MODE_WINDOWED:
			...
			if (!FBitSet (SDL_GetWindowFlags (host.hWnd), SDL_WINDOW_MAXIMIZED))
				...
				if (center_window)
					SDL_SetWindowPosition (host.hWnd, SDL_WINDOWPOS_CENTERED_DISPLAY (display_id),
