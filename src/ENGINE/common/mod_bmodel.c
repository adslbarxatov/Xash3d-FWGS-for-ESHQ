...
#include "swaplib.h"		// [FWGS, 01.05.26]
...
typedef struct
	...
	qboolean	hulls32bit;		// [FWGS, 01.09.26] convert 16-bit clipnodes to 32-bit, if needed
	} dbspmodel_t;
...
// [FWGS, 01.05.26] all these macros are ugly af
#if XASH_BIG_ENDIAN
	#define LUMP_SWAP( x ) .swap = x, .swaplen = HLARRAYSIZE( x )
	#define LUMP_SWAP32( x, y ) .swap = x, .swaplen = HLARRAYSIZE( x ), .swap32 = y, .swaplen32 = HLARRAYSIZE( y )
#else
	#define LUMP_SWAP( x )
...
static mlumpstat_t worldstats[HEADER_LUMPS + EXTRA_LUMPS + HLARRAYSIZE (bspxlumps)];
...
// [FWGS, 01.07.26] Returns index of WAD that texture was found in, or -1 if not found
...
static qboolean Mod_CalcMipTexUsesCustomPalette (model_t *mod, dbspmodel_t *bmod, int textureIndex)
	...
	// сalculate the size assuming we are not using a custom palette
	fs_offset_t	size = Mod_CalculateMipTexSize (&mipTex, false);
	fs_offset_t	remainingBytes;

	// сompute next data offset to determine allocated miptex space
...
/***
=============
Mod_PrintWorldStats_f [FWGS, 01.07.26]
...
void Mod_PrintWorldStats_f (void)
	...
	int	totalmemory = 0;
	for (int i = 0; i < HLARRAYSIZE (worldstats); i++)
		{
...
/***
==================
Mod_GetPVSForPoint [FWGS, 01.07.26]
...
/***
=================
Mod_FindModelOrigin [FWGS, 01.07.26]
...
/***
==================
Mod_CheckWaterAlphaSupport [FWGS, 01.07.26]
...
/***
==================
Mod_GetFaceContents [FWGS, 01.02.25]
...
/***
==================
Mod_MakeNormalAxial [FWGS, 01.07.26]
...
/***
==================
Mod_LightMatrixFromTexMatrix [FWGS, 01.07.26]
...
/***
=================
Mod_CalcSurfaceExtents [FWGS, 01.07.26]
...
static void Mod_CalcSurfaceExtents (model_t *mod, msurface_t *surf, const dbspmodel_t *bmod)
	...
	for (int i = 0; i < surf->numedges; i++)
		...
		if (bmod->version == QBSP2_VERSION)
			{
			if (e >= 0)
				v = &mod->vertexes[mod->edges32[e].v[0]];
			else
				v = &mod->vertexes[mod->edges32[-e].v[1]];
			}
		else
			{
			if (e >= 0)
				v = &mod->vertexes[mod->edges16[e].v[0]];
			else
				v = &mod->vertexes[mod->edges16[-e].v[1]];
			}
...
/***
=================
Mod_CalcSurfaceBounds [FWGS, 01.07.26]
...
/***
=================
Mod_CreateFaceBevels [FWGS, 01.07.26]
=================
***/
static void Mod_CreateFaceBevels (model_t *mod, msurface_t *surf, const dbspmodel_t *bmod)
	...
	int		size = sizeof (mfacebevel_t) + surf->numedges * sizeof (struct mplane_t);
...
static void Mod_SetupHull (dbspmodel_t *bmod, model_t *mod, int headnode, int hullnum, model_t *world)
	...
	if (!bmod->isbsp30ext)
		...
		// [FWGS, 01.09.26] only allocate clipnodes array for the base model, only for first hull
	...
	// [FWGS, 01.09.26] fit array to real count
	/*if (bmod->version == QBSP2_VERSION)*/
	if (bmod->hulls32bit)
		{
...
duplicate the basic information for embedded submodels
=================
***/
static void Mod_SetupSubmodels (model_t *mod, dbspmodel_t *bmod)
	...
	for (int i = 0; i < mod->numsubmodels; i++)
		...
		// [FWGS, 01.09.26] counting a real number of clipnodes per each submodel
		/*if (bmod->version == QBSP2_VERSION)*/
		if (bmod->hulls32bit)
...
/***
=================
Mod_LoadSubmodels [FWGS, 01.07.26]
=================
***/
...
static int Mod_LoadEntities_splitstr_handler (char *prev, char *next, void *userdata)
	...
	*next = '\0';
	...
	// make sure that wad really exists
...
/***
=================
Mod_LoadEntities [FWGS, 01.07.26]
=================
***/
...
/***
=================
Mod_LoadVertexes [FWGS, 01.07.26]
=================
***/
...
/***
=================
Mod_LoadMarkSurfaces [FWGS, 01.07.26]
=================
***/
...
static qboolean Mod_SearchForTextureReplacement (char *out, size_t size, const char *modelname,
	...
	for (int i = 0; i < HLARRAYSIZE (subdirs); i++)
		{
		// truncated name
...
static void Mod_LoadTextureData (model_t *mod, dbspmodel_t *bmod, int textureIndex)
	...
	// [FWGS, 01.09.26] check for multi-layered sky texture (quake1 specific)
	...
	// [FWGS, 01.07.26] FIXME: for ENGINE_IMPROVED_LINETRACE we need to load textures on server too
	...
	// [FWGS, 01.07.26] Try WAD texture (force while r_wadtextures is 1)
	...
	// [FWGS, 01.07.26] WAD failed, so use internal texture (if present)
	...
	// [FWGS, 01.07.26] if texture is completely missed
...
/***
=================
Mod_LoadTextures [FWGS, 01.07.26]
=================
***/
...
/***
=================
Mod_LoadSurfaces [FWGS, 01.07.26]
=================
***/
static void Mod_LoadSurfaces (model_t *mod, dbspmodel_t *bmod)
	...
	if ((test_lightsize > 0) && (prev_lightofs != -1) && (next_lightofs != -1) && (next_lightofs != 99999999))
		...
		if ((samples == 1) || (samples == 3))
			...
			{
			Con_DPrintf (S_WARN "lighting invalid samplecount: %g, defaulting to %i\n", samples,
		...
		if (bmod->version == QBSP2_VERSION)
			...
			for (int j = 0; j < 2; j++)
				...
				if (p >= 0)
					...
					out->children_[j] = (mnode_t *)(mod->leafs + (-1 - p));
#else
				if (j == 0)
					{
					if (p >= 0)
...
/***
=================
Mod_LoadClipnodes [FWGS, 01.07.26]
=================
***/
...
/***
=================
Mod_LoadLightVecs [FWGS, 01.12.24]
=================
***/
static void Mod_LoadLightVecs (model_t *mod, dbspmodel_t *bmod)
	...
	if (bmod->deluxdatasize != bmod->lightdatasize)
		...
		// old method
			...
			Mod_LoadLitfile (mod, "dlit", bmod->lightdatasize, &bmod->deluxedata_out, &bmod->deluxdatasize);
...
/***
=================
Mod_LoadLighting [FWGS, 01.07.26]
=================
***/
...
/***
=================
Mod_FindEndOfBSPFile [FWGS, 01.03.26]
...
static fs_offset_t Mod_FindEndOfBSPFile (const byte *mod_base, size_t bufferlen)
	...
	for (int i = 0; i < HLARRAYSIZE (header->lumps); i++)
		{
	...
	if ((header->version == HLBSP_VERSION) && (ext_header->id == IDEXTRAHEADER) && (ext_header->version == EXTRA_VERSION))
		{
		for (int i = 0; i < HLARRAYSIZE (ext_header->lumps); i++)
			{
...
/***
=================
Mod_TestBmodelLumps [FWGS, 01.07.26]
