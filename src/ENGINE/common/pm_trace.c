...
/***
==================
PM_HullPointContents [FWGS, 01.07.26]
==================
***/
int GAME_EXPORT PM_HullPointContents (hull_t *hull, int num, const vec3_t p)
	...
	if (world.version == QBSP2_VERSION)
		...
		while (num >= 0)
			{
			struct mplane_t	*plane = &hull->planes[hull->clipnodes32[num].planenum];
		...
		while (num >= 0)
			{
			struct mplane_t	*plane = &hull->planes[hull->clipnodes16[num].planenum];
...
/***
==================
PM_HullForStudio [FWGS, 01.07.26]
...
loc0:
	...
	/*if (frac < 0.0f)
		frac = 0.0f;*/
	// [FWGS, 01.09.26] inverted comparison also catches NaN (e.g. from a non-finite trace passed
	...
	while (PM_HullPointContents (hull, hull->firstclipnode, mid) == CONTENTS_SOLID)
		...
		/*if (frac < 0.0f)*/
		// [FWGS, 01.09.26] inverted comparison also catches NaN, see above
		if (!(frac >= 0.0f))
			{
			trace->fraction = midf;
...
int PM_TestPlayerPosition (playermove_t *pmove, vec3_t pos, pmtrace_t *ptrace, pfnIgnore pmFilter)
	{
	pmtrace_t	trace = PM_PlayerTraceExt (pmove, pmove->origin, pmove->origin, 0, pmove->numphysent,
		pmove->physents, -1, pmFilter);
	if (ptrace)
		*ptrace = trace;
...
/***
=============
PM_PointContents [FWGS, 01.07.26]
=============
***/
