...
void *R_StudioGetAnim (studiohdr_t *m_pStudioHeader, model_t *m_pSubModel, mstudioseqdesc_t *pseqdesc)
	...
	if (!Mod_CacheCheck ((cache_user_t *)&(paSequences[pseqdesc->seqgroup])))
		...
		// NOTE: here we build real sub-animation filename because stupid user may rename model
		// without recompile
		string filepath;
		...
		Mod_SwapStudioSeqGroupAnims (m_pStudioHeader, pseqdesc, (byte *)paSequences[pseqdesc->seqgroup].data);
		}
...
/***
===============
Mod_StudioTexName [FWGS, 01.07.24]
...
void Mod_InitStudioAPI (void)
	...
	if (pBlendIface && pBlendIface (SV_BLENDING_INTERFACE_VERSION, &pBlendAPI, &gStudioAPI,
		&studio_transform, &studio_bones))
		{
