...
static CVAR_DEFINE (net_ipname, "ip", "localhost", FCVAR_PRIVILEGED,
	"network ip address");
static CVAR_DEFINE (net_iphostport, "ip_hostport", "0", FCVAR_READ_ONLY,
	"network ip host port");
static CVAR_DEFINE (net_hostport, "hostport", "0", FCVAR_READ_ONLY,
	"network default host port");
static CVAR_DEFINE (net_ipclientport, "ip_clientport", "0", FCVAR_READ_ONLY,
	"network ip client port");
static CVAR_DEFINE (net_clientport, "clientport", "0", FCVAR_READ_ONLY,
	"network default client port");
static CVAR_DEFINE (net_fakelag, "fakelag", "0", FCVAR_PRIVILEGED,
...
static CVAR_DEFINE (net_ip6clientport, "ip6_clientport", "0", FCVAR_READ_ONLY,
...
qboolean NET_MakeSocketReuseAddr (int socket_fd)
	{
	uint	opt = 1;
	return !NET_IsSocketError (setsockopt (socket_fd, SOL_SOCKET, SO_REUSEADDR, (const char *)&opt,
		sizeof (opt)));
	}
...
/***
============
NET_GetHostByName [FWGS, 01.07.26]
============
***/
...
/***
====================
NET_StringToFilterAdr [FWGS, 01.07.26]
====================
***/
...
/***
====================
NET_IsReservedAdr [FWGS, 01.07.26]
...
/***
====================
NET_CompareAdrSort [FWGS, 01.07.26]
...
/***
====================
NET_GetLoopPacket [FWGS, 01.07.26]
====================
***/
...
/***
====================
NET_SendLoopPacket [FWGS, 01.07.26]
====================
***/
...
/***
==================
NET_ClearLaggedList [FWGS, 01.07.26]
...
static qboolean NET_GetLong (byte *pData, size_t size, size_t *outSize, size_t splitsize, connprotocol_t proto)
	...
	if ((packet_number < 0) || (packet_count <= 0) || (packet_number >= max_splits) ||
		(packet_count > max_splits) || (packet_number >= packet_count))
		{
	...
	if ((net.split.current_sequence == -1) || (sequence_number != net.split.current_sequence))
		...
		for (int i = 0; i < HLARRAYSIZE (net.split_flags); i++)
			net.split_flags[i] = -1;
...
qboolean NET_GetPacket (netsrc_t sock, netadr_t *from, byte *data, size_t *length)
	...
	if (NET_GetLoopPacket (sock, from, data, length))
		...
		{
		// [FWGS, 01.05.26] round-robin the socket to prevent constant activity on one starving the other
...
static int NET_SendLong (netsrc_t sock, int net_socket, const char *buf, size_t len, int flags,
	...
	if ((splitsize > sizeof (SPLITPACKET)) && (sock == NS_SERVER) && (len > splitsize))
		...
		while (len > 0)
			...
			int	ret = sendto (net_socket, packet, size + sizeof (SPLITPACKET), flags,
				(const struct sockaddr *)to, tolen);
			if (ret < 0)
				return ret;	// error
			if (ret >= size)
...
/***
==================
NET_SendPacketEx [FWGS, 01.07.26]
==================
***/
...
/***
================
NET_DetermineLocalAddress [FWGS, 01.07.26]
...
void NET_Config (qboolean multiplayer, qboolean changeport)
	...
	if (multiplayer)
		...
		// open sockets
		if (net.allow_ip)
			NET_OpenIP (changeport, net.ip_sockets, net_ipname.string, net_iphostport.value,
				net_ipclientport.value, AF_INET);

		if (net.allow_ip6)
			NET_OpenIP (changeport, net.ip6_sockets, net_ip6name.string, net_ip6hostport.value,
				net_ip6clientport.value, AF_INET6);

		// [FWGS, 01.07.26] validate sockets for dedicated
		if (Host_IsDedicated ())
			{
			/*qboolean nov4, nov6;
