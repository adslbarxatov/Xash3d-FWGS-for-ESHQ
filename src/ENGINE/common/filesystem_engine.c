...
// ESHQ: состо€ние теперь хранитс€ в пам€ти, чтобы посто€нно не дЄргать файл
static unsigned int WAS_Level = 0, WAS_Code = 0;
static unsigned int WAS_gravity = 0xFFFF, WAS_roomtype = 0xFFFF;

// ћетод запрашивает настройки из сохранЄнной конфигурации, обновл€ет их до нового формата
// и контролирует их целостность.
// –ежимы: 0 - достижени€, 1 - гравитаци€, 2 - тип помещени€
// Ќовый уровень учитываетс€ только в режиме 0
void FS_WriteAchievementsScript (byte Mode, int NewLevel)
	...
	// „тение предыдущего состо€ни€ (ошибки игнорируютс€)
	if (!WAS_Code)
		{
	...
	// ѕроверочный код, позвол€ющий избежать посто€нной перезаписи файла и повторного его исполнени€
	newCode = ((gravity >> 4) & 0x3F) | ((roomtype & 0x3F) << 8);

	// ”ровень уже достигнут или €вл€етс€ максимальным -и- проверочный код не изменилс€
	if (((NewLevel <= (int)WAS_Level) || (WAS_Level >= 3)) && (WAS_Code == newCode))
		return;	

	// ѕопытка записать здесь приводила к тому, что в случае одномоментного или близкого к этому
	// срабатывани€ обоих типов триггеров второй из них игнорировалс€ из-за совпадени€ проверочного
	// кода (он к этому моменту уже был актуальным). ѕоэтому изменение версии кода, хран€щегос€ в пам€ти,
	// теперь выполн€етс€ только в св€зке с режимом, в котором вызвана функци€. “ак проверочный код
	// не сможет совпасть с вычисл€емым в реальном времени вариантом, пока все провер€емые им команды
	// не будут выполнены
	// WAS_Code = newCode;

	// ”словие дл€ последующего повышени€.
	// int, потому что иначе при сравнении происходит приведение к uint
	if ((NewLevel > (int)WAS_Level) && (WAS_Level < 3))
		WAS_Level++;

	// «апись проверочного кода (некритично)
