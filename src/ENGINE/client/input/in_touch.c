...
static CVAR_DEFINE_AUTO (touch_sidezone, "0.06", FCVAR_FILTERABLE,
	"side touch zone");
static CVAR_DEFINE_AUTO (touch_pitch, "90", FCVAR_FILTERABLE,
	"touch pitch sensitivity");
static CVAR_DEFINE_AUTO (touch_yaw, "120", FCVAR_FILTERABLE,
	"touch yaw sensitivity");
static CVAR_DEFINE_AUTO (touch_nonlinear_look, "0", FCVAR_FILTERABLE,
	"enable nonlinear touch look");
static CVAR_DEFINE_AUTO (touch_pow_factor, "1.3", FCVAR_FILTERABLE,
	"set > 1 to enable");
static CVAR_DEFINE_AUTO (touch_pow_mult, "400.0", FCVAR_FILTERABLE,
	"power multiplier, usually 200-1000");
static CVAR_DEFINE_AUTO (touch_exp_mult, "0", FCVAR_FILTERABLE,
	"exponent multiplier, usually 20-200, 0 to disable");
static CVAR_DEFINE_AUTO (touch_grid_count, "50", FCVAR_FILTERABLE,
	"touch grid count");
static CVAR_DEFINE_AUTO (touch_grid_enable, "1", FCVAR_FILTERABLE,
	"enable touch grid");
static CVAR_DEFINE_AUTO (touch_config_file, "touch.cfg", FCVAR_ARCHIVE | FCVAR_PRIVILEGED,
	"current touch profile file");
static CVAR_DEFINE_AUTO (touch_precise_amount, "0.5", FCVAR_FILTERABLE,
	"sensitivity multiplier for precise-look");
static CVAR_DEFINE_AUTO (touch_highlight_r, "1.0", 0,
	"highlight r color");
static CVAR_DEFINE_AUTO (touch_highlight_g, "1.0", 0,
	"highlight g color");
static CVAR_DEFINE_AUTO (touch_highlight_b, "1.0", 0,
	"highlight b color");
static CVAR_DEFINE_AUTO (touch_highlight_a, "1.0", 0,
	"highlight alpha");
static CVAR_DEFINE_AUTO (touch_dpad_radius, "1.0", FCVAR_FILTERABLE,
	"dpad radius multiplier");
static CVAR_DEFINE_AUTO (touch_joy_radius, "1.0", FCVAR_FILTERABLE,
	"joy radius multiplier");
static CVAR_DEFINE_AUTO (touch_move_indicator, "0.0", FCVAR_FILTERABLE,
...
/***
=================
Touch_DumpConfig [FWGS, 01.07.26]
...
/***
=================
Touch_ExportConfig_f [FWGS, 01.07.26]
...
/***
=================
Touch_GenerateCode_f [FWGS, 01.07.26]
...
static touch_button_t *Touch_AddButton (touchbuttonlist_t *list, const char *name, const char *texture,
	const char *command, float x1, float y1, float x2, float y2, byte *color, qboolean privileged)
	{
...
// [FWGS, 01.07.26]
void Touch_AddClientButton (const char *name, const char *texture, const char *command, float x1, float y1,
	float x2, float y2, byte *color, int round, float aspect, int flags)
	{
	/*touch_button_t *button;*/
	...
	touch_button_t *button = Touch_AddButton (&touch.list_user, name, texture, command,
		x1, y1, x2, y2, color, true);
...
static void Touch_LoadDefaults_f (void)
	...
	for (int i = 0; i < g_DefaultButtonsLength; i++)
		...
		float 	y1 = g_DefaultButtons[i].y1;
		float	x2 = g_DefaultButtons[i].x2;
		float	y2 = g_DefaultButtons[i].y2;
		...
		if (g_DefaultButtons[i].aspect && g_DefaultButtons[i].round == round_aspect)
			...
			if (g_DefaultButtons[i].texture[0] == '#')
				y2 = y1 + ((float)clgame.scrInfo.iCharHeight / (float)clgame.scrInfo.iHeight) *
					g_DefaultButtons[i].aspect + touch.swidth * 2.0f / refState.height;
			else
		...
		button = Touch_AddButton (&touch.list_user, g_DefaultButtons[i].name, g_DefaultButtons[i].texture,
			g_DefaultButtons[i].command, x1, y1, x2, y2, g_DefaultButtons[i].color, true);
...
// [FWGS, 01.07.26] add default button from client
void Touch_AddDefaultButton (const char *name, const char *texture, const char *command, float x1, float y1,
	float x2, float y2, byte *color, int round, float aspect, int flags)
	{
	/*touchdefaultbutton_t *b;*/

	g_DefaultButtons = Mem_Realloc (touch.mempool, g_DefaultButtons, sizeof (*g_DefaultButtons) *
		(g_DefaultButtonsLength + 1));
...
static void Touch_EnableEdit_f (void)
	...
	// Case A: no config was loaded, touch was generated with lower height, but window was resized higher,
	// reset it to actual size
	...
	// [FWGS, 01.07.26] convert coordinates to actual aspect ratio after it was updated
...
void Touch_Init (void)
	...
	Touch_AddDefaultButton ("use", "touch_default/use", "+use", 0.880000, 0.454457, 1.000000,
		0.681685, color, 2, 1, 0);
	Touch_AddDefaultButton ("jump", "touch_default/jump", "+jump", 0.880000, 0.227228, 1.000000,
		0.454457, color, 2, 1, 0);
	Touch_AddDefaultButton ("attack", "touch_default/shoot", "+attack", 0.760000, 0.530200, 0.880000,
		0.757428, color, 2, 1, 0);
	...
	Touch_AddDefaultButton ("reload", "touch_default/reload", "+reload", 0.000000, 0.302971, 0.120000,
		0.530200, color, 2, 1, 0);
	Touch_AddDefaultButton ("flashlight", "touch_default/flash_light_filled", "impulse 100", 0.920000, 0.000000, 1.000000,
		0.151486, color, 2, 1, 0);
	Touch_AddDefaultButton ("scores", "touch_default/map", "+showscores", 0.760000, 0.000000, 0.840000,
		0.142222, color, 2, 1, 8);
	...
	Touch_AddDefaultButton ("duck", "touch_default/crouch", "+duck", 0.880000, 0.757428, 1.000000,
		0.984656, color, 2, 1, 0);
	Touch_AddDefaultButton ("tduck", "touch_default/tduck", ";+duck", 0.560000, 0.833171, 0.620000,
		0.946785, color, 2, 1, 0);
	Touch_AddDefaultButton ("edit", "touch_default/settings", "touch_enableedit", 0.420000, 0.000000, 0.500000,
		0.151486, color, 2, 1, 32);
	Touch_AddDefaultButton ("menu", "touch_default/menu", "cancelselect", 0.000000, 0.833171, 0.080000,
		0.984656, color, 2, 1, 0);
	Touch_AddDefaultButton ("spray", "touch_default/spray", "impulse 201", 0.680000, 0.000000, 0.760000,
		0.142222, color, 2, 1, 8);
	Touch_AddDefaultButton ("voicechat", "touch_default/microphone", "+voicerecord", 0.780000, 0.817778, 0.860000,
		0.960000, color, 2, 1, 8);
	...
	for (touch_button_t *b = list->first; b; b = b->next)
		...
		if (Touch_IsVisible (b))
			...
			if (FBitSet (b->flags, TOUCH_FL_STROKE))
				...
				ref.dllFuncs.FillRGBA (kRenderTransTexture, x1_, y1_, swidth, y2_ - y1_ - swidth,
					scolor[0], scolor[1], scolor[2], scolor[3]);

				ref.dllFuncs.FillRGBA (kRenderTransTexture, x2_ - swidth, y1_ + swidth,
					swidth, y2_ - y1_ - swidth, scolor[0], scolor[1], scolor[2], scolor[3]);

				ref.dllFuncs.FillRGBA (kRenderTransTexture, x1_ + swidth, y1_,
					x2_ - x1_ - swidth, swidth, scolor[0], scolor[1], scolor[2], scolor[3]);

				ref.dllFuncs.FillRGBA (kRenderTransTexture, x1_, y2_ - swidth,
					x2_ - x1_ - swidth, swidth, scolor[0], scolor[1], scolor[2], scolor[3]);
				}
			}
