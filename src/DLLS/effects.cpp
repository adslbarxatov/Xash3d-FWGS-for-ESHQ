...
void CLaser::StrikeThink (void)
	...
	// ESHQ: непонятно, почему урон не выключался вместе с лазером
	if (!IsOn ())
		return;
...
void CBlood::KeyValue (KeyValueData* pkvd)
	{
	// ESHQ: новая обработка для крови
...
void CBlood::Use (CBaseEntity* pActivator, CBaseEntity* pCaller, USE_TYPE useType, float value)
	...
	if (pev->spawnflags & SF_BLOOD_STREAM)
		UTIL_BloodStream (BloodPosition (pActivator), Direction (),
			(Color () == BLOOD_COLOR_RED) ? 70 : Color (), BloodAmount ());
	else
...
void CMessage::Spawn (void)
	...
	switch (pev->impulse)
		...
		case 1: // Medium radius
			pev->speed = ATTN_MEDIUM;
			break;

		case 2:	// Large radius
			pev->speed = ATTN_LARGE;
			break;

		case 3:	// EVERYWHERE
			pev->speed = ATTN_EVERYWHERE;
			break;

		default:
		case 0: // Small radius
			pev->speed = ATTN_SMALL;
			break;
		}
...
void CItemSoda::CanThink (void)
	{
	EMIT_SOUND (ENT (pev), CHAN_WEAPON, "weapons/g_bounce3.wav", 1, ATTN_MEDIUM);
