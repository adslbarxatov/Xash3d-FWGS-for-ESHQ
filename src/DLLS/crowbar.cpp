...
LINK_ENTITY_TO_CLASS (weapon_spear, CCrowbar);			// ESHQ: поддержка объектов AOMDC
LINK_ENTITY_TO_CLASS (weapon_axe, CCrowbar);
...
void CCrowbar::Spawn ()
	{
	pev->classname = MAKE_STRING ("weapon_crowbar");	// ESHQ: замена имён классов AOMDC
	Precache ();
		...
		if (pEntity)
			...
			if (pEntity->Classify () != CLASS_NONE && pEntity->Classify () != CLASS_MACHINE)
				...
				switch (RANDOM_LONG (0, 2))
					{
					case 0:
						EMIT_SOUND (ENT (m_pPlayer->pev), CHAN_ITEM, "weapons/cbar_hitbod1.wav", 1, ATTN_MEDIUM); break;
					case 1:
						EMIT_SOUND (ENT (m_pPlayer->pev), CHAN_ITEM, "weapons/cbar_hitbod2.wav", 1, ATTN_MEDIUM); break;
					case 2:
