...
void CXenPLight::Spawn (void)
	...
	m_pGlow = CSprite::SpriteCreate (XEN_PLANT_GLOW_SPRITE, pev->origin +
		Vector (0, 0, (pev->mins.z + pev->maxs.z) * 0.5), FALSE);
	m_pGlow->SetTransparency (kRenderGlow, pev->rendercolor.x, pev->rendercolor.y, pev->rendercolor.z, 
...
void CXenSporeLarge::Spawn (void)
	...
	for (int i = 0; i < HLARRAYSIZE (m_hullSizes); i++)
