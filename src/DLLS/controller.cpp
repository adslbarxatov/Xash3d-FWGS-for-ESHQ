...
void CController::HandleAnimEvent (MonsterEvent_t* pEvent)
	...
	switch (pEvent->event)
		...
		case CONTROLLER_AE_BALL_SHOOT:
			...
			// ESHQ: самоповреждение от слишком близкого контакта с жертвой атаки.
			// Необходимо для компенсации атаки зомби лоб-в-лоб
			entvars_t v = pBall->edict ()->v;
			if (m_hEnemy != NULL)
				if ((pev->origin - m_hEnemy->pev->origin).Length2D () < 48)	// > 20 своих + 16 врага + угловой запас
					TakeDamage (&v, &v, 10.0f, DMG_SHOCK);
...
void CController::Spawn ()
	...
	// ESHQ: уменьшены горизонтальные размеры, чтобы зомби могли подойти на расстояние атаки
	...
	{
		{
			...
			HLARRAYSIZE (tlControllerChaseEnemy),
			bits_COND_NEW_ENEMY |
...
Schedule_t	slControllerStrafe[] =
	{
		{
			tlControllerStrafe,
			HLARRAYSIZE (tlControllerStrafe),
			bits_COND_NEW_ENEMY,
			0,
			"ControllerStrafe"
		},
	...
	{
		{
			...
			HLARRAYSIZE (tlControllerTakeCover),
			bits_COND_NEW_ENEMY,
			0,
			"ControllerTakeCover"
		},
...
BOOL CController::CheckRangeAttack2 (float flDot, float flDist)
	{
	// ESHQ: удалён нижний порог flDist для преодоления зацикливания на атаке зомби
	/*if ((flDot > 0.5) && (flDist > 64) && (flDist <= 2048))*/
	if ((flDot > 0.5) && (flDist <= 2048))
		return TRUE;
...
void CControllerHeadBall::HuntThink (void)
	...
	if ((gpGlobals->time - pev->dmgtime > 5) || (pev->renderamt < 64) || (m_hEnemy == NULL) ||
		(m_hOwner == NULL) || (pev->origin.x < -4096) || (pev->origin.x > 4096) || (pev->origin.y < -4096) ||
