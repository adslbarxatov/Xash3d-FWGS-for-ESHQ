...
class CScriptedSentence : public CBaseToggle
	...
	public:
		...
		// ESHQ: поддержка голосовых сообщений
		void Precache (void);
	...
	private:
		int		m_iszSentence;		// message sound
		int		m_iszText;			// message text
...
TYPEDESCRIPTION	CScriptedSentence::m_SaveData[] =
	{
		...
		DEFINE_FIELD (CScriptedSentence, m_iszText, FIELD_STRING),
...
void CScriptedSentence::KeyValue (KeyValueData *pkvd)
	...
	// ESHQ: поддержка голосовых сообщений
		...
		{
		m_iszText = ALLOC_STRING (pkvd->szValue);
		pkvd->fHandled = TRUE;
		}
...
// ESHQ: поддержка голосовых сообщений, в том числе - в виде самостоятельных звуков
...
BOOL CScriptedSentence::StartSentence (CBaseMonster *pTarget)
	...
	// Отображение текста
	UTIL_ShowMessageAll (STRING (m_iszText));
