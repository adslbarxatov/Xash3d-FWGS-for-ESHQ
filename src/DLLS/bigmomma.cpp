	...
	switch (pEvent->event)
		...
		case BIG_AE_MELEE_ATTACK1:
			...
			if (pHurt)
				...
				EMIT_SOUND_DYN (edict (), CHAN_WEAPON, RANDOM_SOUND_ARRAY (pAttackHitSounds), 1.0, ATTN_MEDIUM, 0, 100 + RANDOM_LONG (-5, 5));
				}
			}
			break;
...
void CBigMomma::LayHeadcrab (void)
	...
	EMIT_SOUND_DYN (edict (), CHAN_WEAPON, RANDOM_SOUND_ARRAY (pBirthSounds), 1.0, ATTN_MEDIUM, 0, 100 + RANDOM_LONG (-5, 5));
	m_crabCount++;
	}
...
void CBigMomma::StartTask (Task_t* pTask)
	...
	switch (pTask->iTask)
		...
		case TASK_MELEE_ATTACK1:
			...
			EMIT_SOUND_DYN (ENT (pev), CHAN_VOICE, RANDOM_SOUND_ARRAY (pAttackSounds), 1.0, ATTN_MEDIUM, 0, PITCH_NORM);
...
void CBMortar::Touch (CBaseEntity* pOther)
	...
	EMIT_SOUND_DYN (ENT (pev), CHAN_VOICE, "bullchicken/bc_acid1.wav", 1, ATTN_MEDIUM, 0, iPitch);
