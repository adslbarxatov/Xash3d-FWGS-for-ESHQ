...
void CBarney::BarneyFirePistol (void)
	...
	if (pitchShift > 10)
		...
		pitchShift -= 5;
	EMIT_SOUND_DYN (ENT (pev), CHAN_WEAPON, "barney/ba_attack2.wav", 1, ATTN_MEDIUM, 0, 100 + pitchShift);
	...
	if ((m_MonsterState != MONSTERSTATE_PRONE) && (pevAttacker->flags & FL_CLIENT))
		...
		if (m_hEnemy == NULL)
			...
			if ((m_afMemory & bits_MEMORY_SUSPICIOUS) || IsFacing (pevAttacker, pev->origin))
				...
				PlaySentence ("BA_MAD", 4, VOL_NORM, ATTN_MEDIUM);
				...
				PlaySentence ("BA_SHOT", 4, VOL_NORM, ATTN_MEDIUM);
...
Schedule_t* CBarney::GetSchedule (void)
	...
	if (HasConditions (bits_COND_ENEMY_DEAD) && FOkToSpeak ())
		{
		PlaySentence ("BA_KILL", 4, VOL_NORM, ATTN_MEDIUM);
		}
...
void CBarney::DeclineFollowing (void)
	{
	PlaySentence ("BA_POK", 2, VOL_NORM, ATTN_MEDIUM);
	}
...
// Обгоревший Барни
// =========================================================
class CBurnedBarney: public CBaseMonster
	{
	public:
		void Spawn (void);
		int	Classify (void) { return	CLASS_PLAYER_ALLY; }
...
char* CBurnedBarney::m_szPoses[] = {"lying_on_back", "lying_on_side", "lying_on_stomach"};

void CBurnedBarney::KeyValue (KeyValueData* pkvd)
	{
	if (FStrEq (pkvd->szKeyName, "pose"))
		{
		m_iPose = atoi (pkvd->szValue);
