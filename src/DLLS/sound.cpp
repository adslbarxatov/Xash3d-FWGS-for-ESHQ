...
void CAmbientGeneric::Spawn (void)
	...
	// Set up think function for dynamic modification of ambient sound's pitch or volume. Don't
	// start thinking yet
...
void EMIT_SOUND_SUIT (edict_t *entity, const char *sample)
	...
	if (fvol > 0.05)
		EMIT_SOUND_DYN (entity, CHAN_STATIC, sample, fvol, ATTN_MEDIUM, 0, pitch);
	}
...
void EMIT_GROUPID_SUIT (edict_t *entity, int isentenceg)
	...
	if (fvol > 0.05)
		SENTENCEG_PlayRndI (entity, isentenceg, fvol, ATTN_MEDIUM, 0, pitch);
	}
...
void EMIT_GROUPNAME_SUIT (edict_t *entity, const char *groupname)
	...
	if (fvol > 0.05)
		SENTENCEG_PlayRndSz (entity, groupname, fvol, ATTN_MEDIUM, 0, pitch);
	}
...
float TEXTURETYPE_PlaySound (TraceResult *ptr, Vector vecSrc, Vector vecEnd, int iBulletType)
	...
	switch (chTextureType)
		{
		// ESHQ: обновление звуковой схемы
		default:
		case CHAR_TEX_CONCRETE:
			fvol = 0.9;	fvolbar = 0.4;
			rgsz[0] = "weapons/concrete_hit1.wav";
			rgsz[1] = "weapons/concrete_hit2.wav";
			rgsz[2] = "weapons/concrete_hit3.wav";
			rgsz[3] = "weapons/concrete_hit4.wav";
			cnt = 4;
			break;
	...
	if (pEntity && (FClassnameIs (pEntity->pev, "func_breakable") || FClassnameIs (pEntity->pev, "func_pushable") ||
		...
		{
		// Эти объекты выполняют обработку самостоятельно
		fvol /= 2.0;
		fvolbar /= 2.0;
		}
		...
		if ((ptr->flFraction != 1.0) && RANDOM_LONG (0, 1))
			...
			switch (RANDOM_LONG (0, 1))
				{
				case 0:
					UTIL_EmitAmbientSound (ENT (0), ptr->vecEndPos, "buttons/spark5.wav", flVolume, ATTN_MEDIUM, 0, 100);
					break;
				case 1:
					UTIL_EmitAmbientSound (ENT (0), ptr->vecEndPos, "buttons/spark6.wav", flVolume, ATTN_MEDIUM, 0, 100);
					break;
				}
			}
		}
	// ESHQ: поддержка искр от металла
	...
	UTIL_EmitAmbientSound (ENT (0), ptr->vecEndPos, rgsz[RANDOM_LONG (0, cnt - 1)], fvol, fattn, 0, 96 + RANDOM_LONG (0, 8));
