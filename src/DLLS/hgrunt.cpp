...
#define GRUNT_ATTN						ATTN_MEDIUM	// attenutation of grunt sentences
...
void CHGrunt::GibMonster (void)
	...
	// ESHQ: эта зона, видимо, вообще не работает при текущих моделях
	if (GetBodygroup (2) != 2)
		{
		...
		if (pGun)
			{
			// ESHQ: чтобы оружие не застревало в стенах, уберём разброс по горизонтальным осям
			// и расположим оружие прямо над телом
...
BOOL CHGrunt::CheckMeleeAttack1 (float flDot, float flDist)
	...
	int wpn = pEnemy->Classify ();
	if ((flDist <= 64) && (flDot >= 0.7) &&
		(wpn != CLASS_ALIEN_BIOWEAPON) && (wpn != CLASS_PLAYER_BIOWEAPON))
		{
		return TRUE;
		}
...
BOOL CHGrunt::CheckRangeAttack2 (float flDot, float flDist)
	...
	if (!FBitSet (m_hEnemy->pev->flags, FL_ONGROUND) && (m_hEnemy->pev->waterlevel == 0) &&
		...
		{
		// BUGBUG - we should make this check movetype and make sure it isn't FLY?
		// Players who jump a lot are unlikely to be grenaded.
	...
	if (FBitSet (pev->weapons, HGRUNT_HANDGRENADE))
		...
		// estimate position
		if (HasConditions (bits_COND_SEE_ENEMY))
			vecTarget = vecTarget + ((vecTarget - pev->origin).Length () / gSkillData.hgruntGrenadeSpeed) * 
				m_hEnemy->pev->velocity;
		}
	...
	if (FOkToSpeak () && (g_fGruntQuestion || RANDOM_LONG (0, 1)))
		...
		if (!g_fGruntQuestion)
			...
			switch (RANDOM_LONG (0, 2))
				{
				case 0: // check in
					SENTENCEG_PlayRndSz (ENT (pev), "HG_CHECK", HGRUNT_SENTENCE_VOLUME, ATTN_MEDIUM, 0, m_voicePitch);
					g_fGruntQuestion = 1;
					break;

				case 1: // question
					SENTENCEG_PlayRndSz (ENT (pev), "HG_QUEST", HGRUNT_SENTENCE_VOLUME, ATTN_MEDIUM, 0, m_voicePitch);
					g_fGruntQuestion = 2;
					break;

				case 2: // statement
					SENTENCEG_PlayRndSz (ENT (pev), "HG_IDLE", HGRUNT_SENTENCE_VOLUME, ATTN_MEDIUM, 0, m_voicePitch);
					break;
				}
			}
		else
			...
			switch (g_fGruntQuestion)
				{
				case 1: // check in
					SENTENCEG_PlayRndSz (ENT (pev), "HG_CLEAR", HGRUNT_SENTENCE_VOLUME, ATTN_MEDIUM, 0, m_voicePitch);
					break;

				case 2: // question 
					SENTENCEG_PlayRndSz (ENT (pev), "HG_ANSWER", HGRUNT_SENTENCE_VOLUME, ATTN_MEDIUM, 0, m_voicePitch);
					break;
				}
...
void CHGrunt::Shoot (void)
	...
	Vector vecShellVelocity = gpGlobals->v_right * RANDOM_FLOAT (40, 90) + gpGlobals->v_up * 
		RANDOM_FLOAT (75, 200) + gpGlobals->v_forward * RANDOM_FLOAT (-40, 40);
...
void CHGrunt::HandleAnimEvent (MonsterEvent_t* pEvent)
	...
	switch (pEvent->event)
		...
		case HGRUNT_AE_RELOAD:
			EMIT_SOUND (ENT (pev), CHAN_WEAPON, "hgrunt/gr_reload1.wav", 1, ATTN_MEDIUM);
		...
		case HGRUNT_AE_GREN_TOSS:
			...
			m_fThrowGrenade = FALSE;
			// ESHQ: такое поведение сделано постоянным
			m_flNextGrenadeCheck = gpGlobals->time + RANDOM_FLOAT (3, 6);
			}
			break;
		...
		case HGRUNT_AE_BURST1:
			...
			if (FBitSet (pev->weapons, HGRUNT_9MMAR))
				...
				// ESHQ: подавление зацикливания стрельбы при резкой потере контакта с противником
				if (m_cAmmoLoaded <= 0)
					m_IdealActivity = ACT_RELOAD;

				// the first round of the three round burst plays the sound and puts a sound in the world sound list
				if (RANDOM_LONG (0, 1))
					EMIT_SOUND (ENT (pev), CHAN_WEAPON, "hgrunt/gr_mgun1.wav", 1, ATTN_MEDIUM);
				else
					EMIT_SOUND (ENT (pev), CHAN_WEAPON, "hgrunt/gr_mgun2.wav", 1, ATTN_MEDIUM);
				}
			else
				{
				Shotgun ();
				...
				EMIT_SOUND (ENT (pev), CHAN_WEAPON, "weapons/sbarrel1.wav", 1, ATTN_MEDIUM);
				}
...
void CHGrunt::Spawn ()
	...
	// this is true when the grunt spawns, because he hasn't encountered an enemy yet
...
void CHGrunt::PainSound (void)
	...
	switch (RANDOM_LONG (0, 6))
		{
		case 0:
			EMIT_SOUND (ENT (pev), CHAN_VOICE, "hgrunt/gr_pain1.wav", 1, ATTN_MEDIUM);
			break;
		case 1:
			EMIT_SOUND (ENT (pev), CHAN_VOICE, "hgrunt/gr_pain2.wav", 1, ATTN_MEDIUM);
			break;
		case 2:
			EMIT_SOUND (ENT (pev), CHAN_VOICE, "hgrunt/gr_pain3.wav", 1, ATTN_MEDIUM);
			break;
		case 3:
			EMIT_SOUND (ENT (pev), CHAN_VOICE, "hgrunt/gr_pain4.wav", 1, ATTN_MEDIUM);
			break;
		case 4:
			EMIT_SOUND (ENT (pev), CHAN_VOICE, "hgrunt/gr_pain5.wav", 1, ATTN_MEDIUM);
			break;
		}
...
Schedule_t	slGruntVictoryDance[] =
	{
		{
			tlGruntVictoryDance,
			HLARRAYSIZE (tlGruntVictoryDance),
			bits_COND_NEW_ENEMY |
	...
	{
		{
			...
			HLARRAYSIZE (tlGruntEstablishLineOfFire),
			bits_COND_NEW_ENEMY |
...
Schedule_t	slGruntCombatFace[] =
	{
		{
			tlGruntCombatFace1,
			HLARRAYSIZE (tlGruntCombatFace1),
			bits_COND_NEW_ENEMY |
			bits_COND_ENEMY_DEAD |
			bits_COND_CAN_RANGE_ATTACK1 |
			bits_COND_CAN_RANGE_ATTACK2,
	...
	{
		{
			...
			HLARRAYSIZE (tlGruntSignalSuppress),
			bits_COND_ENEMY_DEAD |
			bits_COND_LIGHT_DAMAGE |
			bits_COND_HEAVY_DAMAGE |
			bits_COND_HEAR_SOUND |
...
Schedule_t	slGruntWaitInCover[] =
	{
		{
			tlGruntWaitInCover,
			HLARRAYSIZE (tlGruntWaitInCover),
			bits_COND_NEW_ENEMY |
			bits_COND_HEAR_SOUND |
			bits_COND_CAN_RANGE_ATTACK1 |
			bits_COND_CAN_RANGE_ATTACK2 |
	...
	{
		{
			...
			HLARRAYSIZE (tlGruntGrenadeCover1),
			0,
			0,
			"GrenadeCover"
		},
	...
	{
		{
			...
			HLARRAYSIZE (tlGruntTossGrenadeCover1),
			0,
			0,
			"TossGrenadeCover"
		},
	...
	{
		{
			...
			HLARRAYSIZE (tlGruntTakeCoverFromBestSound),
			0,
			0,
	...
	{
		{
			...
			HLARRAYSIZE (tlGruntRangeAttack1A),
			bits_COND_NEW_ENEMY |
			bits_COND_ENEMY_DEAD |
			bits_COND_HEAVY_DAMAGE |
	...
	{
		{
			...
			HLARRAYSIZE (tlGruntRangeAttack1B),
			bits_COND_NEW_ENEMY |
			bits_COND_ENEMY_DEAD |
			bits_COND_HEAVY_DAMAGE |
			bits_COND_ENEMY_OCCLUDED |
...
Schedule_t	slGruntRangeAttack2[] =
	{
		{
			tlGruntRangeAttack2,
			HLARRAYSIZE (tlGruntRangeAttack2),
			0,
			0,
			"RangeAttack2"
		},
...
Schedule_t	slGruntRepelAttack[] =
	{
		{
			tlGruntRepelAttack,
			HLARRAYSIZE (tlGruntRepelAttack),
			bits_COND_ENEMY_OCCLUDED,
			0,
			"Repel Attack"
		},
	...
	if (HasConditions (bits_COND_HEAR_SOUND))
		...
		if (pSound)
			...
			if (pSound->m_iType & bits_SOUND_DANGER)
				...
				// Currently, this is the grunt's signal that a grenade has landed nearby,
	...
	switch (m_MonsterState)
		...
		case MONSTERSTATE_COMBAT:
			...
			if (HasConditions (bits_COND_NEW_ENEMY))
				...
				if (InSquad ())
					...
					if (!IsLeader ())
						...
						{
						// the leader of a squad of grunts has just seen the player or a 
						...
						if (FOkToSpeak ())
							...
							if ((m_hEnemy != NULL) && m_hEnemy->IsPlayer ())
								SENTENCEG_PlayRndSz (ENT (pev), "HG_ALERT", HGRUNT_SENTENCE_VOLUME, GRUNT_ATTN,
									0, m_voicePitch);
							...
							// monster
								...
								SENTENCEG_PlayRndSz (ENT (pev), "HG_MONST", HGRUNT_SENTENCE_VOLUME, GRUNT_ATTN,
									0, m_voicePitch);
			...
			// no ammo
				...
				{
				// this individual just realized he's out of bullet ammo. 
			...
			// damaged just a little
				...
				if ((iPercent <= 90) && (m_hEnemy != NULL))
					...
					// this grunt was hit and is going to run to cover
					if (FOkToSpeak ())
			...
			// can't see enemy
				...
				if (HasConditions (bits_COND_CAN_RANGE_ATTACK2) && OccupySlot (bits_SLOTS_HGRUNT_GRENADE))
					{
					// this grunt is about to throw or fire a grenade at the player.
					// Great place for "fire in the hole" "frag out" etc
					if (FOkToSpeak ())
						{
					...
					{
					// grunt cannot see the enemy and has just decided to 
					...
					{
					// grunt is going to stay put for a couple seconds to see if
...
Schedule_t* CHGrunt::GetScheduleOfType (int Type)
	...
	switch (Type)
		...
		case SCHED_TAKE_COVER_FROM_ENEMY:
			...
			if (InSquad ())
				{
				if (g_iSkillLevel == SKILL_HARD && HasConditions (bits_COND_CAN_RANGE_ATTACK2) &&
					OccupySlot (bits_SLOTS_HGRUNT_GRENADE))
					{
					if (FOkToSpeak ())
						{
...
class CHGruntRepel: public CBaseMonster
	...
	public:
		void Spawn (void);
		void Precache (void);
		void HLEXPORT RepelUse (CBaseEntity* pActivator, CBaseEntity* pCaller, USE_TYPE useType, float value);
...
// =========================================================
class CBurnedHGrunt: public CBaseMonster
	{
	public:
		void Spawn (void);
		int	Classify (void) { return	CLASS_HUMAN_MILITARY; }
...
char* CBurnedHGrunt::m_szPoses[] = {"deadstomach", "deadside", "deadsitting"};

void CBurnedHGrunt::KeyValue (KeyValueData* pkvd)
	{
	if (FStrEq (pkvd->szKeyName, "pose"))
		{
		m_iPose = atoi (pkvd->szValue);
...
LINK_ENTITY_TO_CLASS (monster_hgrunt_burned, CBurnedHGrunt);

void CBurnedHGrunt::Spawn (void)
	{
	PRECACHE_MODEL ("models/hgrunt.mdl");
	SET_MODEL (ENT (pev), "models/hgrunt.mdl");
