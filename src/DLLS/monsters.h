...
#define SF_MONSTER_OWN_SEQUENCE			1024	// ESHQ: поддержка блокировки поведения
...
#define IMPLEMENT_CUSTOM_SCHEDULES(derivedClass, baseClass)\
		...
		{\
			Schedule_t *pSchedule = ScheduleInList( pName, m_scheduleList, HLARRAYSIZE(m_scheduleList) );\
			if ( !pSchedule )\
