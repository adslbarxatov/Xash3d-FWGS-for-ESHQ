...
// ESHQ: изменение поведени€ (0 - человеческие, 1 - чужие малые, 2 - чужие большие)
...
BOOL CBaseMonster::HasHumanGibs (void)
	...
	if (myClass == CLASS_HUMAN_MILITARY ||
		...
		// ESHQ: новый класс дл€ human assassin
...
void CBaseMonster::GibMonster (void)
	...
	// ESHQ: обход необрабатываемого самоназначени€ размеров дл€ сущностей, созданных MonsterInitDead
	float sizeCoeff;
	float soundNumber;
	if (ENT (pev)->v.deadflag == DEAD_ORIGINALLY)	// ѕризнак инициализации через MonsterInitDead
		{
		sizeCoeff = 1.0f;
		}

	// ESHQ: новый звук дл€ расчленЄнки, зависимый от размера объекта
	else
		{
		...
		if (v.z > 2.0f)	// ѕризнак живого существа
			sizeCoeff /= 8.5f;	// sqrt (72)
		}

	// Ќормализованный на единицу коэффициент подт€гиваетс€ к ней корнем, после чего пересчитываетс€ на частоту
	...
	// –азворот частот (меньший размер = больша€ частота) с отражением диапазона относительно значени€ 100
	sizeCoeff = 130.0f - (sizeCoeff - 70.0f);

	switch ((int)soundNumber)
		{
		// ќт короткого к длинному
		case 0:
			EMIT_SOUND_DYN (ENT (pev), CHAN_WEAPON, "common/bodysplat2.wav", 1.0f,
				ATTN_MEDIUM, 0, sizeCoeff);
			break;

		// ESHQ: этот звук пока не удаЄтс€ найти
		/*case 1:
		...
		case 2:*/
		default:	// ¬озможные 3.0f и чуть более
...
void CGib::BounceGibTouch (CBaseEntity* pOther)
	...
	if (pev->flags & FL_ONGROUND)
		...
		if ((m_material != matNone) && (RANDOM_LONG (0, 2) == 0))
			...
			CBreakable::MaterialSoundRandom (edict (), (Materials2)m_material, volume);
			}
		}
	}
...
void CGib::Spawn (const char* szGibModel)
	...
	// ESHQ: возможно, это больше не потребуетс€.
	// ”стран€ет бесконечное прыгание гибсов, хот€ остальные последстви€ неизвестны
...
int CBaseMonster::TakeDamage (entvars_t* pevInflictor, entvars_t* pevAttacker, float flDamage, int bitsDamageType)
	...
	// ESHQ: сброс фиксированного sequence
	if (pev->spawnflags & SF_MONSTER_OWN_SEQUENCE)
		pev->spawnflags &= ~SF_MONSTER_OWN_SEQUENCE;
