...
int CBaseMonster::Save (CSave& save)
	...
	if (!CBaseToggle::Save (save))
		return 0;
	return save.WriteFields ("CBaseMonster", this, m_SaveData, HLARRAYSIZE (m_SaveData));
	}
...
int CBaseMonster::Restore (CRestore& restore)
	...
	if (!CBaseToggle::Restore (restore))
		return 0;
	int status = restore.ReadFields ("CBaseMonster", this, m_SaveData, HLARRAYSIZE (m_SaveData));
...
void CBaseMonster::MakeIdealYaw (Vector vecTarget)
	...
	// ESHQ: блокировка при фиксированном поведении
	if (pev->spawnflags & SF_MONSTER_OWN_SEQUENCE)
		return;
...
void CBaseMonster::HandleAnimEvent (MonsterEvent_t* pEvent)
	...
	switch (pEvent->event)
		...
		case SCRIPT_EVENT_SOUND:			// Play a named wave file
			EMIT_SOUND (edict (), CHAN_BODY, pEvent->options, 1.0, ATTN_SMALL);
			break;

		case SCRIPT_EVENT_SOUND_VOICE:
			EMIT_SOUND (edict (), CHAN_VOICE, pEvent->options, 1.0, ATTN_SMALL);
			break;
		...
		case MONSTER_EVENT_SWISHSOUND:
			...
			EMIT_SOUND (ENT (pev), CHAN_BODY, "zombie/claw_miss2.wav", 1, ATTN_MEDIUM);
			break;
			}
...
void CBaseMonster::SentenceStop (void)
	{
	EMIT_SOUND (edict (), CHAN_VOICE, "common/null.wav", 1.0, ATTN_SMALL);
	}
...
void CBaseMonster::MonsterInitDead (void)
	...
	// Copy health
	pev->max_health = pev->health;
	pev->deadflag = DEAD_ORIGINALLY;	// ESHQ: выделенное состояние для управления звуком
...
CBaseEntity* CBaseMonster::DropItem (char* pszItemName, const Vector& vecPos, const Vector& vecAng)
	...
	if (pItem)
		{
		// ESHQ: при создании объекта принудительное расположение над инициатором и запрет на боковые смещения
		pItem->pev->origin = Vector (pev->origin.x, pev->origin.y, pItem->pev->origin.z);	
		pItem->pev->velocity = Vector (0, 0, RANDOM_FLOAT (0, 100));						
