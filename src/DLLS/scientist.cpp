...
#define		NUM_SCIENTIST_BODIES	5	// ESHQ: п€тое - обгоревшее
...
Schedule_t	slFaceTargetScared[] =
	{
		{
			tlFaceTargetScared,
			HLARRAYSIZE (tlFaceTargetScared),
			bits_COND_HEAR_SOUND |
			bits_COND_NEW_ENEMY,
			bits_SOUND_DANGER,
			"FaceTargetScared"
...
Schedule_t	slScientistStartle[] =
	{
		{
			tlScientistStartle,
			HLARRAYSIZE (tlScientistStartle),
			bits_COND_NEW_ENEMY |
			bits_COND_SEE_ENEMY |
			bits_COND_SEE_HATE |
			bits_COND_SEE_FEAR |
...
void CScientist::DeclineFollowing (void)
	...
	PlaySentence ("SC_POK", 2, VOL_NORM, ATTN_MEDIUM);
	}
...
void CScientist::Scream (void)
	...
	if (FOkToSpeak ())
		...
		Talk (10);
		m_hTalkTarget = m_hEnemy;
		PlaySentence ("SC_SCREAM", RANDOM_FLOAT (3, 6), VOL_NORM, ATTN_MEDIUM);
		}
	}
	...
	switch (pTask->iTask)
		...
		case TASK_SAY_FEAR:
			if (FOkToSpeak ())
				...
				Talk (2);
				m_hTalkTarget = m_hEnemy;
				// ESHQ: удалЄн вызов, в котором при возврате из функции [CBaseEntity * EHANDLE :: operator -> ()]
				// значени€ NULL почему-то происходит обращение к адресу 0x00000000 (в регистр EAX попадает
				// нулевое значение)
...
void CScientist::HandleAnimEvent (MonsterEvent_t* pEvent)
	...
	switch (pEvent->event)
		...
		case SCIENTIST_AE_NEEDLEON:
			...
			// ¬идимо, список parts достигаетс€ путЄм переволнени€ списка body
			}
			break;
	...
	switch (m_MonsterState)
		...
		case MONSTERSTATE_IDLE:
			if (pEnemy)
				...
				// After 15 seconds of being hidden, return to alert
			...
			if (IsFollowing ())
				...
				if ((relationship != R_DL) && (relationship != R_HT))
					...
					// Just face and follow
					return GetScheduleOfType (SCHED_TARGET_FACE);
					}
				else
					{
					// I just saw something new and scary, react
					if (HasConditions (bits_COND_NEW_ENEMY))
						// React to something scary
						return GetScheduleOfType (SCHED_FEAR);

					// face and follow, but I'm scared!
		...
		case MONSTERSTATE_COMBAT:
			...
			// Cower and panic from the scary sound!
			if (HasConditions (bits_COND_HEAR_SOUND))
...
BOOL CScientist::CanHeal (void)
	...
	if ((m_healTime > gpGlobals->time) || (m_hTargetEnt == NULL) ||
		(m_hTargetEnt->pev->health > (m_hTargetEnt->pev->max_health * 0.5)))
		return FALSE;
...
char* CDeadScientist::m_szPoses[] = {"lying_on_back", "lying_on_stomach", "dead_sitting", "dead_hang", 
...
int CSittingScientist::FIdleSpeak (void)
	...
	if (pentFriend && RANDOM_LONG (0, 1))
		...
		SENTENCEG_PlayRndSz (ENT (pev), m_szGrp[TLK_PQUESTION], 1.0, ATTN_SMALL, 0, pitch);
		// set global min delay for next conversation
		CTalkMonster::g_talkWaitTime = gpGlobals->time + RANDOM_FLOAT (4.8, 5.2);
		return TRUE;
		}
	...
	if (RANDOM_LONG (0, 1))
		{
		SENTENCEG_PlayRndSz (ENT (pev), m_szGrp[TLK_PIDLE], 1.0, ATTN_SMALL, 0, pitch);
		// set global min delay for next conversation
		CTalkMonster::g_talkWaitTime = gpGlobals->time + RANDOM_FLOAT (4.8, 5.2);
		return TRUE;
		}
...
char* CBurnedScientist::m_szPoses[] = {"lying_on_back", "lying_on_stomach", "dead_sitting", "dead_hang", 
	"dead_table1", "dead_table2", "dead_table3"};

void CBurnedScientist::KeyValue (KeyValueData* pkvd)
	{
	if (FStrEq (pkvd->szKeyName, "pose"))
		{
		m_iPose = atoi (pkvd->szValue);
...
LINK_ENTITY_TO_CLASS (monster_scientist_burned, CBurnedScientist);

void CBurnedScientist::Spawn ()
	{
	PRECACHE_MODEL ("models/scientist.mdl");
	SET_MODEL (ENT (pev), "models/scientist.mdl");
	...
	pev->body = 4;	// ќбгоревший скин
	pev->skin = 0;	// –уки не измен€ютс€
