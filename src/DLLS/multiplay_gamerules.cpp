...
BOOL CHalfLifeMultiplay::GetNextBestWeapon (CBasePlayer* pPlayer, CBasePlayerItem* pCurrentWeapon)
	...
	for (i = 0; i < MAX_ITEM_TYPES; i++)
		...
		while (pCheck)
			...
			else if ((pCheck->iWeight () > iBestWeight) && (pCheck != pCurrentWeapon))
				{
		...
		while (1)
			...
			if (IS_MAP_VALID (szMap))
				...
				if (hasbuffer)
					{
					// FWGS
					s1 = g_engfuncs.pfnInfoKeyValue (szBuffer, "minplayers");
					if (s1 && s1[0])
						{
						item->minplayers = atoi (s1);
						item->minplayers = max (item->minplayers, 0);
						item->minplayers = min (item->minplayers, gpGlobals->maxClients);
						}

					s2 = g_engfuncs.pfnInfoKeyValue (szBuffer, "maxplayers");
					if (s2 && s2[0])
						{
						item->maxplayers = atoi (s2);
