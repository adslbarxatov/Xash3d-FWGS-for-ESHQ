...
// ESHQ: переопределение имЄн переменных, в которых будут хранитьс€ звуки
...
class CBaseDoor: public CBaseToggle
	...
	public:
		...
		BYTE	m_bStopSnd;			// ESHQ: звук полного открыти€
		BYTE	m_bStop2Snd;		// ESHQ: звук полного закрыти€
...
void CBaseDoor::Spawn ()
	...
	// ESHQ: новый механизм запирани€ usable-дверей
	if (FBitSet (pev->spawnflags, SF_DOOR_USE_ONLY))
		{
		if (!FStringNull (pev->targetname))
			pev->body = 0;	// »спользуем как флаг "замка"
		else
			pev->body = 1;	// ѕри отсутствии имени объекта замок разблокируетс€
...
void CBaseDoor::Precache (void)
	...
	switch (m_bMoveSnd)
		{
		// ESHQ: нова€ обработка звукового сопровождени€
		case	0:
		default:
	...
	switch (m_bStop2Snd)
		{
		// ќтсутствие звука определ€етс€ состо€нием -1
		case	-1:
			pev->noiseReturned = ALLOC_STRING ("common/null.wav");
			break;

		// ѕри совпадении со звуком открыти€ (обратна€ совместимость)
		case	0:
			pev->noiseReturned = ALLOC_STRING ((char*)STRING (pev->noiseArrived));
			break;

		// ќстальные Ц как обычно
		case	1:
		case	2:
		case	3:
		case	4:
...
void CBaseDoor::DoorGoUp (void)
	...
	if (!FBitSet (pev->spawnflags, SF_DOOR_SILENT))
		{
		// ESHQ: исправление дл€ одинаковых звуков закрыти€ и открыти€
...
void CBaseDoor::Blocked (CBaseEntity* pOther)
	...
	// ESHQ: остановка предыдущего звука (без этого циклические звуки "застревают" в двер€х)
	if (!FBitSet (pev->spawnflags, SF_DOOR_SILENT))
		{
		STOP_SOUND (ENT (pev), CHAN_STATIC, (char*)STRING (pev->noiseMoving));
		}
...
void CRotDoor::Spawn (void)
	...
	if (FBitSet (pev->spawnflags, SF_DOOR_START_OPEN))
		{
		// ESHQ: исправл€ем ошибку с обменом переменных (исходный вариант устанавливал им одинаковые значени€)
	...
	// ESHQ: if the door is flagged for USE button activation only, use NULL touch function
	if (FBitSet (pev->spawnflags, SF_DOOR_USE_ONLY))
		{
		if (!FStringNull (pev->targetname))
			pev->body = 0;	// »спользуем как флаг "замка"
...
class CMomentaryDoor: public CBaseToggle
	...
	public:
		...
		float	oldSoundValue;		// ESHQ: значение, используемое дл€ контрол€ состо€ни€ звука двери
	};
...
void CMomentaryDoor::Use (CBaseEntity* pActivator, CBaseEntity* pCaller, USE_TYPE useType, float value)
	...
	if ((value != 0) && (fabs ((double)value) < 1.0))
		...
		// –азные знаки означают, что изменено направление движени€.
		// ¬ обоих случа€х звук нужно перезапустить
	...
	// «вук остановки невозможен в начале движени€
	...
	oldSoundValue = value;			// ќбновление значени€
	}
