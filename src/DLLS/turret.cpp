...
void CBaseTurret::Precache ()
	...
	PRECACHE_SOUND ("turret/tu_die1.wav");
...
void CBaseTurret::Ping (void)
	...
	if (m_flPingTime == 0)
		...
		{
		m_flPingTime = gpGlobals->time + 1;
		EMIT_SOUND (ENT (pev), CHAN_ITEM, "turret/tu_ping.wav", 1, ATTN_MEDIUM);
		EyeOn ();
		}
...
void CMiniTurret::Shoot (Vector& vecSrc, Vector& vecDirToEnemy)
	...
	switch (RANDOM_LONG (0, 2))
		{
		case 0: 
			EMIT_SOUND (ENT (pev), CHAN_WEAPON, "weapons/hks1.wav", 1, ATTN_MEDIUM); 
			break;
		case 1: 
			EMIT_SOUND (ENT (pev), CHAN_WEAPON, "weapons/hks2.wav", 1, ATTN_MEDIUM); 
			break;
		case 2: 
			EMIT_SOUND (ENT (pev), CHAN_WEAPON, "weapons/hks3.wav", 1, ATTN_MEDIUM); 
			break;
		}
...
void CBaseTurret::Deploy (void)
	...
	if (pev->sequence != TURRET_ANIM_DEPLOY)
		...
		EMIT_SOUND (ENT (pev), CHAN_BODY, "turret/tu_deploy.wav", TURRET_MACHINE_VOLUME, ATTN_MEDIUM);
...
void CBaseTurret::Retire (void)
	...
	if (!MoveTurret ())
		...
		if (m_iSpin)
			...
			EMIT_SOUND_DYN (ENT (pev), CHAN_BODY, "turret/tu_deploy.wav", TURRET_MACHINE_VOLUME, ATTN_MEDIUM, 0, 120);
...
void CTurret::SpinUpCall (void)
	...
	if (!m_iSpin)
		...
		if (!m_iStartSpin)
			...
			EMIT_SOUND (ENT (pev), CHAN_BODY, "turret/tu_spinup.wav", TURRET_MACHINE_VOLUME, ATTN_MEDIUM);
			m_iStartSpin = 1;
			pev->framerate = 0.1;
			}
...
void CTurret::SpinDownCall (void)
	...
	if (pev->framerate == 1.0)
		...
		EMIT_SOUND (ENT (pev), CHAN_ITEM, "turret/tu_spindown.wav", TURRET_MACHINE_VOLUME, ATTN_MEDIUM);
		}
	pev->framerate -= 0.02;
...
void CBaseTurret::TurretDeath (void)
	...
	if (pev->deadflag != DEAD_KILLED)
		...
		EMIT_SOUND (ENT (pev), CHAN_BODY, "turret/tu_die1.wav", 1.0, ATTN_MEDIUM);
		EMIT_SOUND_DYN (ENT (pev), CHAN_STATIC, "turret/tu_active2.wav", 0, 0, SND_STOP, 100);
	...
	if (pev->dmgtime + RANDOM_FLOAT (0, 5) > gpGlobals->time)
		{
		Vector vecSrc = Vector (RANDOM_FLOAT (pev->absmin.x, pev->absmax.x), 
			RANDOM_FLOAT (pev->absmin.y, pev->absmax.y), 0);
		if (m_iOrientation == 0)
...
void CBaseTurret::TraceAttack (entvars_t* pevAttacker, float flDamage, Vector vecDir, TraceResult* ptr, 
	int bitsDamageType)
	{
	if (ptr->iHitgroup == 10)
		{
...
int CBaseTurret::TakeDamage (entvars_t* pevInflictor, entvars_t* pevAttacker, float flDamage, int bitsDamageType)
	...
	if (pev->health <= 10)
		{
		if (m_iOn && ((0x1 | RANDOM_LONG (0, 0x7FFF)) > 800))
			{
			m_fBeserk = 1;
			SetThink (&CBaseTurret::SearchThink);
			}
