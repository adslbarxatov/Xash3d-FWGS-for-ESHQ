...
float CBaseToggle::AxisValue (int flags, const Vector& angles)
	{
	if (FBitSet (flags, SF_DOOR_ROTATE_Xa))
		return angles.z;
	if (FBitSet (flags, SF_DOOR_ROTATE_Ya))
		return angles.x;
...
void CBaseToggle::AxisDir (entvars_t* pev)
	{
	if (FBitSet (pev->spawnflags, SF_DOOR_ROTATE_Xa))
		pev->movedir = Vector (0, 0, 1);	// around z-axis
	else if (FBitSet (pev->spawnflags, SF_DOOR_ROTATE_Ya))
