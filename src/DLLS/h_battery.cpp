...
void CRecharge::Use (CBaseEntity* pActivator, CBaseEntity* pCaller, USE_TYPE useType, float value)
	...
	if ((m_iJuice <= 0) || (!(pActivator->pev->weapons & (1 << WEAPON_SUIT))))
		...
		if (m_flSoundTime <= gpGlobals->time)
			...
			EMIT_SOUND (ENT (pev), CHAN_ITEM, "items/suitchargeno1.wav", 0.85, ATTN_MEDIUM);
			}
		return;
		}
	...
	if (!m_iOn)
		{
		m_iOn++;
		EMIT_SOUND (ENT (pev), CHAN_ITEM, "items/suitchargeok1.wav", 0.85, ATTN_MEDIUM);
		m_flSoundTime = 0.56 + gpGlobals->time;
		}
	if ((m_iOn == 1) && (m_flSoundTime <= gpGlobals->time))
		{
		m_iOn++;
		EMIT_SOUND (ENT (pev), CHAN_STATIC, "items/suitcharge1.wav", 0.85, ATTN_MEDIUM);
		}
