...
void CBasePlayer::WaterMove ()
	...
	if (!RANDOM_LONG (0, 0x1f) && (RANDOM_LONG (0, AIRTIME - 1) >= air))
		...
		switch (RANDOM_LONG (0, 3))
			{
			case 0:	
				EMIT_SOUND (ENT (pev), CHAN_BODY, "player/pl_swim1.wav", 0.8, ATTN_MEDIUM); 
				break;

			case 1:	
				EMIT_SOUND (ENT (pev), CHAN_BODY, "player/pl_swim2.wav", 0.8, ATTN_MEDIUM); 
				break;

			case 2:	
				EMIT_SOUND (ENT (pev), CHAN_BODY, "player/pl_swim3.wav", 0.8, ATTN_MEDIUM); 
				break;

			case 3:	
				EMIT_SOUND (ENT (pev), CHAN_BODY, "player/pl_swim4.wav", 0.8, ATTN_MEDIUM); 
				break;
			}
		}

	if (pev->watertype == CONTENTS_LAVA)		// FWGS: do damage
		{
		if (pev->dmgtime < gpGlobals->time)
			TakeDamage (VARS (eoNullEntity), VARS (eoNullEntity), 10 * pev->waterlevel, DMG_BURN);
		}
	else if (pev->watertype == CONTENTS_SLIME)	// FWGS: do damage
		{
		...
		if (!FNullEnt (tr.pHit))
			...
			if (pEntity->Classify () == CLASS_PLAYER)
				...
				if (g_pGameRules->PlayerRelationship (this, pEntity) == GR_TEAMMATE)
					...
					// No need to get it % based since 100 it's the max
					}
...
void CBasePlayer::PostThink ()
	...
	if ((FBitSet (pev->flags, FL_ONGROUND)) && (pev->health > 0) && m_flFallVelocity >= PLAYER_FALL_PUNCH_THRESHHOLD)
		...
		if (pev->watertype == CONTENTS_WATER)	// FWGS
			{
...
void CRevertSaved::Use (CBaseEntity* pActivator, CBaseEntity* pCaller, USE_TYPE useType, float value)
	{
	// ESHQ: поддержка ликвидации игрока при вызове объекта
