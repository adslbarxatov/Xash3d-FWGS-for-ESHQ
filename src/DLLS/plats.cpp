...
void CBasePlatTrain::Precache (void)
	...
	switch (m_bStopSnd)
		...
		case	3:
			...
			break;
		case	4:
			PRECACHE_SOUND ("plats/heavystop1.wav");
			pev->noiseArrived = MAKE_STRING ("plats/heavystop1.wav");
			break;
		case	5:
...
void CFuncPlat::HitBottom (void)
	...
	if (pev->noiseStopMoving)
		EMIT_SOUND (ENT (pev), CHAN_WEAPON, (char*)STRING (pev->noiseStopMoving), m_volume, ATTN_MEDIUM);
...
void CFuncTrain::Spawn (void)
	...
	// ESHQ: поддержка сверхдальнего охвата звуком
	if (FBitSet (pev->spawnflags, SF_TRAIN_LARGERADIUS))
		m_flAttenuation = 0.3f;
	else if (FBitSet (pev->spawnflags, SF_TRAIN_BIGRADIUS))
...
void CFuncTrackTrain::StopSound (void)
	...
	if (m_soundPlaying && pev->noise)
		...
		EMIT_SOUND_DYN (ENT (pev), CHAN_ITEM, "plats/ttrain_brake1.wav", m_flVolume, ATTN_MEDIUM, 0, 100);
		}
...
void CFuncTrackTrain::UpdateSound (void)
	...
	if (!m_soundPlaying)
		...
		EMIT_SOUND_DYN (ENT (pev), CHAN_ITEM, "plats/ttrain_start1.wav", m_flVolume, ATTN_MEDIUM, 0, 100);
		EMIT_SOUND_DYN (ENT (pev), CHAN_STATIC, (char*)STRING (pev->noise), m_flVolume, ATTN_MEDIUM, 0, (int)flpitch);
		m_soundPlaying = 1;
		}
	else
		{
...
void CFuncTrackTrain::Precache (void)
	...
	switch (m_sounds)
		...
		case 1: 
			PRECACHE_SOUND ("plats/ttrain1.wav"); pev->noise = MAKE_STRING ("plats/ttrain1.wav");
			break;
		case 2: 
			PRECACHE_SOUND ("plats/ttrain2.wav"); pev->noise = MAKE_STRING ("plats/ttrain2.wav"); 
			break;
		case 3: 
			PRECACHE_SOUND ("plats/ttrain3.wav"); pev->noise = MAKE_STRING ("plats/ttrain3.wav"); 
			break;
		case 4: 
			PRECACHE_SOUND ("plats/ttrain4.wav"); pev->noise = MAKE_STRING ("plats/ttrain4.wav"); 
			break;
		case 5: 
			PRECACHE_SOUND ("plats/ttrain6.wav"); pev->noise = MAKE_STRING ("plats/ttrain6.wav");
			break;
		case 6: 
			PRECACHE_SOUND ("plats/ttrain7.wav"); pev->noise = MAKE_STRING ("plats/ttrain7.wav"); 
			break;
		}
...
void CFuncTrackChange::Use (CBaseEntity* pActivator, CBaseEntity* pCaller, USE_TYPE useType, float value)
	...
	if (m_code == TRAIN_BLOCKING)
		...
		EMIT_SOUND (ENT (pev), CHAN_VOICE, "buttons/button11.wav", 1, ATTN_MEDIUM);
		return;
		}
