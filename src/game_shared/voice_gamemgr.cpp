...
// These store the masks we last sent to each client so we can determine if
CPlayerBitVec	g_SentGameRulesMasks[VOICE_MAX_PLAYERS];

// we need to resend them
CPlayerBitVec	g_SentBanMasks[VOICE_MAX_PLAYERS];
...
bool CVoiceGameMgr::PlayerHasBlockedPlayer (CBasePlayer *pReceiver, CBasePlayer *pSender)
	...
	if ((iReceiverIndex < 0) || (iReceiverIndex >= m_nMaxPlayers) || (iSenderIndex < 0) ||
		(iSenderIndex >= m_nMaxPlayers))
		return false;
